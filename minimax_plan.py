...
def build_subject_definitions(char_slots, ref_image_slots, ref_video_segs, ref_audio_segs):
    ...
    for slot_index, slot in enumerate(char_slots):
        ...
        line = "<Subject %d> is the replacement subject shown in %s" % (subject, pictures)
        if description:
            line += ", which is %s" % description.rstrip(" .")
        lines.append(line + ".")

    for i, _seg in enumerate(ref_video_segs):
        lines.append("<Video %d> is the source video providing the camera movement, "
                     "environment, lighting, composition and action choreography."
                     % (i + 1))
...
def compile_continuous_minimax(global_prompt, shots, soundscape="", music="",
                               instruction="", subject_lines=None,
                               retention_lines=None):
    """Compile all timeline prompts as one flowing, uninterrupted action sequence.

    The timeline still determines ordering, but its action beats belong to one [Shot 1].
    Later shot numbers and cut timestamps would imply editorial cuts that are not there.
    """
    parts = []
    ...
    if written:
        body.append("This is one continuous uninterrupted take with the same camera, "
                    "scene, subjects, lighting, and motion continuity throughout. "
                    "Do not cut, reset, jump in time, change viewpoint abruptly, or "
                    "restart the action between the following beats.")
        for index, shot in enumerate(written):
            text = shot["prompt"].strip().rstrip(" .")
            if index == 0:
                body.append("%s." % text)
            elif index == len(written) - 1:
                body.append("Finally, while remaining in the same uninterrupted take, %s." % text)
            else:
                body.append("Then, continuing naturally without a cut, %s." % text)

    if body:
        parts.append(("detailed_description: " if subject_lines
                      else "integrated_multimodal_description: ") + "[Shot 1] " + " ".join(body))
    if (soundscape or "").strip():
        parts.append("overall_soundscape: " + soundscape.strip())
    parts.append("non_diegetic_music: " + ((music or "").strip() or "N/A"))
    return "\n\n".join(parts).strip()
...
def plan_timeline(tdata, win_start, duration_frames, fps, global_prompt="",
    ...
    if retake:
        ...
        # DHan presents Ref2VA as a single visible media/prompt lane. Text blocks
        # created there live in motionSegments so they remain visible alongside
        # Ref2VA media, but semantically they are ordinary storyboard shots.
        if ref_mode_on:
            seen_ids = {str(s.get("id", "")) for s in segments}
            for s in (tdata.get("motionSegments", []) or []):
                if s.get("type") != "text" or not overlaps(s, win_start, win_end):
                    continue
                sid = str(s.get("id", ""))
    ...
    if ref_mode_on:
        ...
        # The Director exposes three global Voice Reference slots below the subject
        # cards. They are independent of @charN and share H3's native three-audio cap
        # with the timed REF AUDIO lane. Voice references are placed first so their
        # <Audio N> numbering stays stable; timeline audio uses whatever capacity remains.
        for vr in (tdata.get("voiceReferences", []) or []):
            if not isinstance(vr, dict) or not vr.get("name"):
                continue
            if len(ref_audio_segs) >= MAX_REF_AUDIOS:
                break
            ref_audio_segs.append({
                "audioFile": vr.get("name"),
                "fileName": vr.get("displayName") or vr.get("name"),
                "_voice_ref": True,
            })

        if use_custom_audio and len(ref_audio_segs) < MAX_REF_AUDIOS:
            ...
            remaining = MAX_REF_AUDIOS - len(ref_audio_segs)
            ref_audio_segs.extend(audio[:remaining])
    ...
    if prompt_format == FORMAT_MINIMAX:
        ...
        # FL2VA always samples the joint H3 video+audio latent, but when the prompt
        # carries no audio intent at all the model can legitimately converge toward
        # near-silence. During the earlier startup-delay debugging we mistakenly
        # treated this as an audio-VAE/lazy-input problem. Keep FL2VA's native AV
        # path untouched and provide only a neutral diegetic-audio fallback when the
        # user supplied no explicit soundscape. Explicit Audio:/overall_soundscape
        # text still wins, and Retake/Ref2VA are not changed here.
        if not ref_mode_on and not retake and not soundscape:
            soundscape = (
                "Natural synchronized diegetic audio matching the visible scene and "
                "actions; preserve realistic room tone, movement sounds, and any "
                "clearly implied vocal reactions. No added music unless requested."
            )
        ...
        if subject_lines:
            ...
            if named and not ref_video_segs:
                ...
                retention_lines.append(
                    ...
                    "shot." % named)
            if ref_video_segs:
                videos = ", ".join("<Video %d>" % (i + 1) for i in range(len(ref_video_segs)))
                summary = (
                    "[video editing + reference generation] The target video is an edited "
                    "version of %s. Replace the original depicted people with %s derived "
                    "from the supplied pictures." % (videos, named)
                )
                retention_lines.append(
                    "%s (source video): partially_preserved - preserve the background "
                    "environment, composition, camera path, lighting, non-target objects, "
                    "action choreography and the original people's screen-space motion paths; "
                    "discard only their visual identities." % videos)
                retention_lines.append(
                    "%s: fully_preserved - preserve the identity, face and appearance from "
                    "the supplied pictures." % named)
        ...
        if not ref_mode_on:
            ...
            instruction = alignment_instruction(
                ...
                written_shots if retake or ref_video_segs else 1, actual_seconds)
        # Normal FL2VA / Ref2VA timeline prompts are action beats in one continuous
        # take. Retake remains a single marked replacement range using the established
        # storyboard formatting.
        if retake or ref_video_segs:
            prompt = compile_storyboard_minimax(global_prompt, shots, soundscape, music,
                                                subject_lines, retention_lines, instruction, summary)
        else:
            prompt = compile_continuous_minimax(global_prompt, shots, soundscape, music,
                                                instruction, subject_lines, retention_lines)
    else:
