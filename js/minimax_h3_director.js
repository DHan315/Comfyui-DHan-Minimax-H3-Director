...
const HIDDEN_WIDGET_NAMES = ["timeline_data", "local_prompts", "segment_lengths", "guide_strength", "audio_data", "use_custom_audio", "inpaint_audio", "use_custom_motion", "override_audio", "voice_reference_audio", "voice_lora_name", "voice_lora_strength", "voice_identity_guidance", "voice_reference_seconds", "global_prompt", "negative_prompt", "negative_prompting", "start_second", "end_second", "duration_seconds", "start_frame", "end_frame", "duration_frames", "epsilon", "divisible_by", "img_compression"];
  ...
  try {
    if (jsonStr) {
      ...
      if (Array.isArray(p.characters)) {
        parsed.characters = p.characters.slice(0, 3).map(c => ({
          images: Array.isArray(c?.images) ? c.images.slice(0, 2) : [],
...
class TimelineEditor {
  constructor(node, container, domWidget) {
    ...
    this._groupResizeInitialSegments = null;
    this._groupResizeBounds = null;
    this._groupResizePreviewTimelines = null;
    this._lastWidth = 0;
  ...
  getMultiSelectionBounds(sourceTimelines = null) {
    if (!this.isMultiSelectActive()) return null;
    const selected = new Set(this.selectedSegmentIds || []);
    const sources = sourceTimelines || {
      image: this.timeline.segments,
      motion: this.timeline.motionSegments,
      audio: this.timeline.audioSegments,
    };
    let minStart = Infinity, maxEnd = -Infinity, count = 0;
    for (const track of ["image", "motion", "audio"]) {
      for (const seg of (sources?.[track] || [])) {
        if (!selected.has(seg.id)) continue;
        const start = Number(seg.start) || 0;
        const end = start + Math.max(1, Number(seg.length) || 1);
        minStart = Math.min(minStart, start);
        maxEnd = Math.max(maxEnd, end);
        count++;
      }
    }
    if (!count || !Number.isFinite(minStart) || !Number.isFinite(maxEnd) || maxEnd <= minStart) return null;
    return { start: minStart, end: maxEnd, length: maxEnd - minStart };
  }

  getMultiGroupHandleHit(mouseX, mouseY) {
    if (!this.isMultiSelectActive() || this.retakeMode) return null;
    if (mouseY < RULER_HEIGHT || mouseY > this.canvasHeight) return null;
    const bounds = this.getMultiSelectionBounds(this._groupResizePreviewTimelines || this._multiDragPreviewTimelines || null);
    if (!bounds) return null;
    const width = this.canvas.offsetWidth;
    const totalFrames = this.getVisualDurationFrames();
    if (!width || totalFrames <= 0) return null;
    const x1 = (bounds.start / totalFrames) * width;
    const x2 = (bounds.end / totalFrames) * width;
    const threshold = Math.max(HANDLE_HIT_PX, 12);
    if (Math.abs(mouseX - x1) <= threshold) return { dir: "left", bounds };
    if (Math.abs(mouseX - x2) <= threshold) return { dir: "right", bounds };
    return null;
  }

  buildGroupResizePreview(mouseFrame, dir) {
    const initial = this._groupResizeInitialSegments;
    const bounds = this._groupResizeBounds;
    if (!initial || !bounds || bounds.length <= 0) return null;
    const selected = new Set(this.selectedSegmentIds || []);
    const minGroupFrames = Math.max(MIN_SEGMENT_LENGTH, 2);
    let newStart = bounds.start, newEnd = bounds.end;
    if (dir === "right") newEnd = Math.max(bounds.start + minGroupFrames, mouseFrame);
    else newStart = Math.max(0, Math.min(bounds.end - minGroupFrames, mouseFrame));

    const scale = Math.max(minGroupFrames, newEnd - newStart) / Math.max(1, bounds.length);
    const mapTrack = arr => (arr || []).map(seg => {
      if (!selected.has(seg.id)) return { ...seg };
      const oldStart = Number(seg.start) || 0;
      const oldEnd = oldStart + Math.max(1, Number(seg.length) || 1);
      let s, e;
      if (dir === "right") {
        s = newStart + (oldStart - bounds.start) * scale;
        e = newStart + (oldEnd - bounds.start) * scale;
      } else {
        s = newEnd - (bounds.end - oldStart) * scale;
        e = newEnd - (bounds.end - oldEnd) * scale;
      }
      s = Math.max(0, Math.round(s));
      e = Math.round(e);
      return { ...seg, start: s, length: Math.max(MIN_SEGMENT_LENGTH, e - s) };
    });
    return { image: mapTrack(initial.image), motion: mapTrack(initial.motion), audio: mapTrack(initial.audio) };
  }
  ...
  updateSelectionFromBox() {
    ...
    for (const track of ["image", "motion", "audio"]) {
      ...
      if (track === "image") {
        ...
        trackBottom = (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight);
      } else if (track === "audio") {
        trackTop = (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight);
        trackBottom = (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) + this.audioTrackHeight;
      } else if (track === "motion") {
        trackTop = this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT : RULER_HEIGHT + this.blockHeight + this.audioTrackHeight;
        trackBottom = trackTop + this.motionTrackHeight;
      }
  ...
  growTimelineIfNeeded(requiredFrames) {
    // Ref2VA media are global H3 references, so their source duration must not
    // silently change how many output frames the user asked to generate.
    if (this._dhanH3Mode === "Ref2VA") return;
  ...
  getTrackFromY(y) {
    if (y > (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) + this.audioTrackHeight) return "motion";
    if (y > (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight)) return "audio";
    return "image";
  }
  ...
  loadMedia() {
    const restoreImageObject = (seg) => {
      if (!seg || seg.imgObj) return;

      let src = "";
      if (seg.imageFile) {
        // imageFile is the persistent ComfyUI input path. Rebuild a fresh /view URL
        // after browser refresh instead of relying on an old blob/data URL.
        const parts = String(seg.imageFile).replace(/\\/g, "/").split("/");
        const filename = parts.pop() || "";
        const subfolder = parts.join("/");
        if (filename) {
          src = api.apiURL(
      ...
      } else if (seg.imageB64 && !String(seg.imageB64).startsWith("blob:")) {
        src = seg.imageB64;
      }
      ...
      seg.imgObj.onerror = () => {
        // Keep the persistent imageFile intact so a later reload can retry.
        seg.imgObj = null;
      };
      seg.imgObj.src = src;
    };
  ...
  createDOM() {
    ...
    // --- Compact MiniMax H3 settings strip ---
    // Reorganizes the existing native widgets into two compact columns.
    // These are proxies for the same Python widgets (no new settings/state).
    const compactSettings = document.createElement("div");
    compactSettings.className = "dhan-h3-compact-settings";
    compactSettings.style.cssText =
      "display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:0 0 7px 0;" +
      "padding:7px 8px;background:#1d1d1d;border:1px solid #111;border-radius:5px;" +
      "box-sizing:border-box;min-width:0;";

    const makeSection = (title) => {
      const section = document.createElement("div");
      section.style.cssText =
        "display:grid;grid-template-columns:minmax(72px,110px) minmax(90px,1fr);" +
        "gap:4px 7px;align-items:center;min-width:0;";
      const head = document.createElement("div");
      head.textContent = title;
      head.style.cssText =
        "grid-column:1 / -1;color:#7f91a7;font-size:9px;font-weight:700;" +
        "letter-spacing:.08em;text-transform:uppercase;margin:0 0 2px 0;";
      section.appendChild(head);
      compactSettings.appendChild(section);
      return section;
    };

    const leftSettings = makeSection("Resolution");
    const rightSettings = makeSection("Timing / H3");
    const compactBindings = [];

    const fireCompactWidget = (widget, value) => {
      if (!widget) return;
      widget.value = value;
      if (widget.callback) {
        try { widget.callback(value, app.canvas, this.node, null, null); }
        catch (_) { try { widget.callback(value); } catch (_) { } }
      }
      app.graph?.setDirtyCanvas?.(true, true);
    };

    const makeCompactControl = (section, widgetName, label, kind = "number") => {
      const widget = this.node.widgets?.find(w => w.name === widgetName);
      if (!widget) return null;

      const labelEl = document.createElement("div");
      labelEl.textContent = label;
      labelEl.style.cssText = "font-size:10px;color:#aaa;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;";
      ...
      if (kind === "select") {
        ...
        ctrl.addEventListener("change", () => fireCompactWidget(widget, ctrl.value));
      } else {
        ctrl = document.createElement("input");
        ctrl.type = "number";
        if (widget.options?.min !== undefined) ctrl.min = widget.options.min;
        if (widget.options?.max !== undefined) ctrl.max = widget.options.max;
        if (widget.options?.step !== undefined) ctrl.step = widget.options.step;
        ctrl.value = widget.value ?? 0;
        ctrl.addEventListener("change", () => {
          const parsed = kind === "int" ? parseInt(ctrl.value || "0", 10) : parseFloat(ctrl.value || "0");
          fireCompactWidget(widget, Number.isFinite(parsed) ? parsed : 0);
          ctrl.value = widget.value ?? 0;
        });
      }

      ctrl.style.cssText =
        "width:100%;min-width:0;height:22px;box-sizing:border-box;background:#292929;color:#ddd;" +
        "border:1px solid #111;border-radius:3px;padding:2px 6px;font-size:10px;";
      section.appendChild(labelEl);
      section.appendChild(ctrl);
      compactBindings.push({ widget, ctrl, kind });
      return { ctrl, labelEl, widget };
    };

    makeCompactControl(leftSettings, "custom_width", "Width", "int");
    makeCompactControl(leftSettings, "custom_height", "Height", "int");
    makeCompactControl(leftSettings, "frame_rate", "Frame rate", "number");
    makeCompactControl(leftSettings, "resize_method", "Resize", "select");

    const modeCompact = makeCompactControl(rightSettings, "model_type", "Mode", "select");
    const startCompact = makeCompactControl(rightSettings, "start_second", "Start", "number");
    const endCompact = makeCompactControl(rightSettings, "end_second", "End", "number");
    const durationCompact = makeCompactControl(rightSettings, "duration_seconds", "Duration", "number");
    makeCompactControl(rightSettings, "shift_video", "Video shift", "number");
    makeCompactControl(rightSettings, "shift_audio", "Audio shift", "number");

    // Start/End are useful when Ref2VA is consuming reference media, but are
    // unnecessary clutter for FL2VA. Keep the underlying widgets intact and
    // only change their compact presentation.
    const refreshCompactMode = () => {
      const mode = String(this.node.widgets?.find(w => w.name === "model_type")?.value || "FL2VA").toUpperCase();
      const showRange = mode === "REF2VA";
      const showDuration = mode !== "RETAKE";
      for (const entry of [startCompact, endCompact]) {
        if (!entry) continue;
        entry.labelEl.style.display = showRange ? "" : "none";
        entry.ctrl.style.display = showRange ? "" : "none";
      }
      if (durationCompact) {
        durationCompact.labelEl.style.display = showDuration ? "" : "none";
        durationCompact.ctrl.style.display = showDuration ? "" : "none";
      }
    };
    if (modeCompact?.ctrl) {
      modeCompact.ctrl.addEventListener("change", () => requestAnimationFrame(refreshCompactMode));
    }
    refreshCompactMode();

    // Keep proxy controls synchronized if values are changed elsewhere (load timeline,
    // callbacks, converted inputs, etc.). Existing widgets remain the source of truth.
    this._syncCompactH3Settings = () => {
      for (const { widget, ctrl } of compactBindings) {
        const value = widget?.value;
        if (value !== undefined && String(ctrl.value) !== String(value)) {
    ...
    // Hide only the native rows represented above. If a setting is converted to a
    // connected input, leave the native/input presentation alone.
    const compactWidgetNames = [
      "custom_width", "custom_height", "frame_rate", "resize_method",
      "model_type", "start_second", "end_second", "duration_seconds",
      "shift_video", "shift_audio"
    ];
    for (const name of compactWidgetNames) {
      const linkedInput = this.node.inputs?.find(i => i.name === name && i.link != null);
      const widget = this.node.widgets?.find(w => w.name === name);
      if (widget && !linkedInput) hideWidget(widget);
    }

    // Vue nodes mode can ignore the legacy hidden flag and still paint native widget rows.
    // Collapse these three timing widgets using ComfyUI's converted-widget convention;
    // they remain serialized/backend values and the compact controls above stay authoritative.
    const hardHideNativeTimingRows = () => {
      for (const name of ["start_second", "end_second", "duration_seconds", "resize_method"]) {
        const linkedInput = this.node.inputs?.find(i => i.name === name && i.link != null);
        const w = this.node.widgets?.find(x => x.name === name);
        if (!w || linkedInput) continue;
        hideWidget(w);
        if (!w._dhanOriginalType) w._dhanOriginalType = w.type;
    ...
    hardHideNativeTimingRows();
    this.node._dhanH3HardHideTimingRows = hardHideNativeTimingRows;

    // ComfyUI's newer Vue frontend can keep widget-backed input rows visible even
    // when the widget itself is hidden. Remove only the unlinked duplicate rows
    // that are represented by the compact panel; the widgets remain the backend
    // source of truth and linked inputs are never touched.
    const duplicateCompactInputs = new Set([
      "start_second", "end_second", "duration_seconds", "resize_method"
    ]);
    if (Array.isArray(this.node.inputs)) {
      for (let i = this.node.inputs.length - 1; i >= 0; i--) {
        const input = this.node.inputs[i];
        if (duplicateCompactInputs.has(input?.name) && input.link == null) {
    ...
    uploadAudioBtn.innerHTML = `${ICONS.audio} Add Ref Audio`;
    ...
    const refImageFileInput = document.createElement("input");
    refImageFileInput.type = "file";
    refImageFileInput.accept = "image/*";
    refImageFileInput.multiple = true;
    refImageFileInput.style.display = "none";
    refImageFileInput.addEventListener("change", (e) => this.handleMotionUpload(e.target.files));
    this.refImageFileInput = refImageFileInput;

    const uploadRefImageBtn = document.createElement("button");
    uploadRefImageBtn.className = "pr-btn";
    uploadRefImageBtn.innerHTML = `${ICONS.upload} Add Ref Image`;
    uploadRefImageBtn.addEventListener("click", () => refImageFileInput.click());
    this.uploadRefImageBtn = uploadRefImageBtn;
    ...
    actionGroup.appendChild(refImageFileInput);
    ...
    actionGroup.appendChild(uploadRefImageBtn);
    ...
    // H3 Director keeps only timeline-generic controls here.
    // Retake/start/end/mark/help are inherited LTX/DHan controls and are hidden
    // until an H3-specific implementation exists.
    ...
    this.mainTrackLabel = createTrackLabel("FL2VA", "#1e1e1e", "main", this.mainTrackEnabled, () => {
    ...
    this.audioTrackLabel = createTrackLabel("REF AUDIO", "#1e1e1e", "audio", this.audioTrackEnabled, () => {
    ...
    this.motionTrackLabel = createTrackLabel("REF2VA", "#1e1e1e", "motion", this.motionTrackEnabled, () => {
    ...
    this.wrapper.appendChild(compactSettings);
    this.wrapper.appendChild(toolbar);
    this.wrapper.appendChild(this.layoutContainer);

    // Persistent voice-reference panel (separate from the visual timeline).
    const voicePanel = document.createElement("div");
    voicePanel.style.display = "grid";
    voicePanel.style.gridTemplateColumns = "110px minmax(160px, 1fr) minmax(190px, 1fr) auto";
    voicePanel.style.alignItems = "center";
    voicePanel.style.gap = "8px";
    voicePanel.style.padding = "8px 10px";
    voicePanel.style.background = "#1e1e1e";
    voicePanel.style.border = "1px solid #111";
    voicePanel.style.borderRadius = "4px";

    const voiceTitle = document.createElement("div");
    voiceTitle.textContent = "VOICE REF";
    voiceTitle.style.fontSize = "11px";
    voiceTitle.style.fontWeight = "700";
    voiceTitle.style.color = "#ccc";

    const voiceAudioSelect = document.createElement("select");
    voiceAudioSelect.style.width = "100%";
    voiceAudioSelect.style.background = "#292929";
    voiceAudioSelect.style.color = "#ddd";
    voiceAudioSelect.style.border = "1px solid #111";
    voiceAudioSelect.style.borderRadius = "3px";
    voiceAudioSelect.style.padding = "5px";

    const voiceLoraSelect = document.createElement("select");
    voiceLoraSelect.style.width = "100%";
    voiceLoraSelect.style.background = "#292929";
    voiceLoraSelect.style.color = "#ddd";
    voiceLoraSelect.style.border = "1px solid #111";
    voiceLoraSelect.style.borderRadius = "3px";
    voiceLoraSelect.style.padding = "5px";

    const voiceDropBtn = document.createElement("button");
    voiceDropBtn.className = "pr-btn";
    voiceDropBtn.textContent = "Add Voice";

    const voiceFileInput = document.createElement("input");
    voiceFileInput.type = "file";
    voiceFileInput.accept = "audio/*,video/*";
    voiceFileInput.style.display = "none";

    const populateFromWidget = (select, widget) => {
      select.innerHTML = "";
      let values = widget?.options?.values || widget?.options?.items || [];
      if (typeof values === "function") values = values();
      if (!Array.isArray(values)) values = [];
      if (widget?.value && !values.includes(widget.value)) values = [widget.value, ...values];
      for (const value of values) {
        const option = document.createElement("option");
        option.value = value;
        option.textContent = value === "none" ? "No voice reference" : value;
        select.appendChild(option);
      }
      if (widget) select.value = widget.value;
    };

    const voiceAudioWidget = this.node.widgets?.find(w => w.name === "voice_reference_audio");
    const voiceLoraWidget = this.node.widgets?.find(w => w.name === "voice_lora_name");
    populateFromWidget(voiceAudioSelect, voiceAudioWidget);
    populateFromWidget(voiceLoraSelect, voiceLoraWidget);

    const setWidget = (widget, value) => {
      if (!widget) return;
      widget.value = value;
      if (widget.callback) widget.callback(value);
      this.node.setDirtyCanvas(true, true);
    };
    voiceAudioSelect.addEventListener("change", () => setWidget(voiceAudioWidget, voiceAudioSelect.value));
    voiceLoraSelect.addEventListener("change", () => setWidget(voiceLoraWidget, voiceLoraSelect.value));
    voiceDropBtn.addEventListener("click", () => voiceFileInput.click());
    voiceFileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        const stored = [result.subfolder, result.name].filter(Boolean).join("/");
        const option = document.createElement("option");
        option.value = stored;
        option.textContent = stored;
        voiceAudioSelect.appendChild(option);
        voiceAudioSelect.value = stored;
        setWidget(voiceAudioWidget, stored);
      } catch (err) {
        console.error("[LTXDirector] Voice reference upload failed", err);
        alert(`Voice reference upload failed: ${err.message || err}`);
      } finally {
        voiceFileInput.value = "";
      }
    });

    voicePanel.appendChild(voiceTitle);
    voicePanel.appendChild(voiceAudioSelect);
    voicePanel.appendChild(voiceLoraSelect);
    voicePanel.appendChild(voiceDropBtn);
    voicePanel.appendChild(voiceFileInput);
    this.wrapper.appendChild(voicePanel);
    ...
    // Negative Prompt controls live directly below Global Prompt.
    const negativeSection = document.createElement("div");
    negativeSection.style.display = "flex";
    negativeSection.style.flexDirection = "column";
    negativeSection.style.gap = "6px";
    negativeSection.style.padding = "8px 10px";
    negativeSection.style.background = "#1e1e1e";
    negativeSection.style.border = "1px solid #111";
    negativeSection.style.borderRadius = "4px";
    negativeSection.style.boxSizing = "border-box";
    negativeSection.style.width = "100%";

    const negativeHeader = document.createElement("div");
    negativeHeader.style.display = "flex";
    negativeHeader.style.alignItems = "center";
    negativeHeader.style.justifyContent = "space-between";
    negativeHeader.style.gap = "10px";

    const negativeTitle = document.createElement("div");
    negativeTitle.textContent = "NEGATIVE PROMPT";
    negativeTitle.style.fontSize = "11px";
    negativeTitle.style.fontWeight = "700";
    negativeTitle.style.color = "#ccc";

    const negativeToggleLabel = document.createElement("label");
    negativeToggleLabel.style.display = "flex";
    negativeToggleLabel.style.alignItems = "center";
    negativeToggleLabel.style.gap = "7px";
    negativeToggleLabel.style.fontSize = "11px";
    negativeToggleLabel.style.color = "#bbb";
    negativeToggleLabel.style.cursor = "pointer";
    negativeToggleLabel.style.userSelect = "none";

    const negativeToggle = document.createElement("input");
    negativeToggle.type = "checkbox";
    negativeToggle.style.margin = "0";
    negativeToggle.style.cursor = "pointer";

    const negativeToggleText = document.createElement("span");
    negativeToggleText.textContent = "OFF";

    negativeToggleLabel.appendChild(negativeToggle);
    negativeToggleLabel.appendChild(negativeToggleText);
    negativeHeader.appendChild(negativeTitle);
    negativeHeader.appendChild(negativeToggleLabel);

    const negativeInput = document.createElement("textarea");
    negativeInput.placeholder = "Enter negative prompt here...";
    negativeInput.spellcheck = false;

    // Do NOT reuse .pr-prompt-area here. That class is designed for the Director's
    // fixed-height prompt wrappers and applies sizing rules that make this standalone
    // field expand into a blank clipped panel.
    negativeInput.style.display = "block";
    negativeInput.style.width = "100%";
    negativeInput.style.height = "82px";
    negativeInput.style.minHeight = "82px";
    negativeInput.style.maxHeight = "220px";
    negativeInput.style.resize = "vertical";
    negativeInput.style.boxSizing = "border-box";
    negativeInput.style.padding = "8px 10px";
    negativeInput.style.margin = "0";
    negativeInput.style.background = "#121212";
    negativeInput.style.color = "#ddd";
    negativeInput.style.border = "1px solid #343434";
    negativeInput.style.borderRadius = "4px";
    negativeInput.style.outline = "none";
    negativeInput.style.fontFamily = "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
    negativeInput.style.fontSize = "11px";
    negativeInput.style.lineHeight = "1.4";
    negativeInput.style.overflow = "auto";
    negativeInput.style.flex = "none";

    const negativePromptWidget = this.node.widgets?.find(w => w.name === "negative_prompt");
    const negativeToggleWidget = this.node.widgets?.find(w => w.name === "negative_prompting");

    const setNegativeWidget = (widget, value) => {
      if (!widget) return;
      const oldValue = widget.value;
      widget.value = value;
      try { this.node.onWidgetChanged?.(widget.name, value, oldValue, widget); } catch (_) {}
      try { widget.callback?.(value); } catch (_) {}
      this.node.setDirtyCanvas?.(true, true);
      window.app?.graph?.change?.();
    };

    const NEGATIVE_EXPANDED_PX = 104;

    const resizeForNegativeUI = (enabled) => {
      // TimelineEditor lives inside one ComfyUI DOM widget whose wrapper is height:100%.
      // Merely revealing the textarea therefore leaves it clipped unless we explicitly
      // grow the LiteGraph node. Track the delta ourselves, then let Comfy re-measure.
      try {
        const applied = !!this._dhanNegativeHeightApplied;

        if (enabled && !applied) {
          const current = this.node?.size?.[1] || 0;
          this.node?.setSize?.([
            this.node?.size?.[0] || 1375,
            current + NEGATIVE_EXPANDED_PX
          ]);
          this._dhanNegativeHeightApplied = true;
        } else if (!enabled && applied) {
          const current = this.node?.size?.[1] || 0;
          this.node?.setSize?.([
            this.node?.size?.[0] || 1375,
            Math.max(100, current - NEGATIVE_EXPANDED_PX)
          ]);
          this._dhanNegativeHeightApplied = false;
        }

        requestAnimationFrame(() => {
          try {
            // Force the timeline DOM widget to see the newly revealed/hidden content.
            const timelineWidget = this.node?.widgets?.find(w => w.name === "timeline_ui");
            const measured = timelineWidget?.computeSize?.(this.node?.size?.[0]);

            if (measured && measured[1] > 0) {
              const baseOtherHeight =
                Math.max(0, (this.node?.computeSize?.()?.[1] || this.node?.size?.[1] || 0) -
                (timelineWidget.last_y || 0));

              // Never shrink below the explicit toggle delta we just applied.
              const targetH = Math.max(
                this.node?.size?.[1] || 0,
                (timelineWidget.last_y || 0) + measured[1] + 8
              );
              if (targetH > 0) {
                this.node?.setSize?.([
                  this.node?.size?.[0] || 1375,
    ...
    const syncNegativeUI = () => {
      const enabled = !!negativeToggle.checked;
      negativeToggleText.textContent = enabled ? "ON" : "OFF";
      negativeToggleText.style.color = enabled ? "#eee" : "#888";
      negativeInput.style.display = enabled ? "block" : "none";
      negativeSection.style.borderColor = enabled ? "#333" : "#111";
      resizeForNegativeUI(enabled);
    };

    negativeToggle.checked = !!negativeToggleWidget?.value;
    negativeInput.value = String(negativePromptWidget?.value || "");
    syncNegativeUI();
    requestAnimationFrame(() => syncNegativeUI());

    negativeToggle.addEventListener("change", () => {
      setNegativeWidget(negativeToggleWidget, !!negativeToggle.checked);
      syncNegativeUI();
    });

    let negativeSaveTimer = null;
    negativeInput.addEventListener("input", () => {
      setNegativeWidget(negativePromptWidget, negativeInput.value);
      if (negativeSaveTimer) clearTimeout(negativeSaveTimer);
    ...
    negativeInput.addEventListener("focus", () => {
      this.wrapper.classList.add("has-focus");
      negativeInput.style.borderColor = "#555";
    });
    negativeInput.addEventListener("blur", () => {
      this.wrapper.classList.remove("has-focus");
      negativeInput.style.borderColor = "#343434";
      if (negativeSaveTimer) clearTimeout(negativeSaveTimer);
    });

    negativeSection.appendChild(negativeHeader);
    negativeSection.appendChild(negativeInput);
    this.wrapper.appendChild(negativeSection);

    this.negativePromptSection = negativeSection;
    this.negativePromptToggle = negativeToggle;
    this.negativePromptInput = negativeInput;
  ...
  // Find the nearest non-overlapping start position to a requested frame.
  // Unlike the drag physics solver, this keeps existing segments fixed and moves
  // only the newly inserted item. This makes file drops land close to the cursor.
  _findNearestFreeStart(segments, requestedStart, length, timelineEnd) {
    const maxStart = Math.max(0, timelineEnd - length);
    const requested = clamp(Math.round(requestedStart), 0, maxStart);
    const occupied = (segments || [])
      .filter(s => s && s.type !== "ghost" && s.type !== "temp")
      .map(s => ({ start: Math.round(s.start), end: Math.round(s.start + s.length) }))
      .sort((a, b) => a.start - b.start);

    const fits = (start) => {
      const end = start + length;
      return occupied.every(r => end <= r.start || start >= r.end);
    };

    if (fits(requested)) return requested;

    // Candidate positions are the cursor location and every segment edge. Pick
    // whichever legal position is closest to where the user dropped the file.
    const candidates = new Set([0, maxStart]);
    for (const r of occupied) {
      candidates.add(clamp(r.end, 0, maxStart));
      candidates.add(clamp(r.start - length, 0, maxStart));
    }
    ...
    // If the visible timeline has no free slot, allow the new segment to extend
    // the timeline rather than moving it back to the beginning.
    return best !== null ? best : Math.max(requested, ...occupied.map(r => r.end), 0);
  }
        ...
        try {
          ...
          img.onload = () => {
            ...
            if (targetFrameStart !== null) {
              // Keep existing segments where they are and place the new image in
              // the nearest available slot to the drop cursor.
              newStart = this._findNearestFreeStart(
                this.timeline.segments,
                newStart,
                newLength,
                currentDuration
              );
              targetFrameStart = newStart + newLength; // Place batch items sequentially.
            }
  ...
  // larger files use /minimax_director_upload_chunk to bypass the 413 size limit.
  async _uploadVideoFile(file) {
    ...
    if (file.size > CHUNK_SIZE) {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        if (resp.status !== 200) throw new Error(`MiniMax Director video chunk upload failed: ${resp.status}`);
        if (i === totalChunks - 1) {
          const data = await resp.json();
          return data.name;
        }
  ...
  async handleVideoUpload(files, targetFrameStart = null) {
    ...
    if (this.retakeMode) {
      ...
      if (this.timeline.retakeVideo) {
        ...
        if (oldVid.videoEl) {
          // Intentional cleanup: detach media handlers before clearing src so the
          // browser does not report the reset as an unsupported-codec failure.
          oldVid.videoEl.onerror = null;
          oldVid.videoEl.onloadeddata = null;
          oldVid.videoEl.onloadedmetadata = null;
          oldVid.videoEl.onseeked = null;
          oldVid.videoEl.pause();
          oldVid.videoEl.removeAttribute("src");
          oldVid.videoEl.load();
        }
        if (oldVid._blobUrl) {
      ...
      await new Promise(async (resolve) => {
        try {
          ...
          if (isImage) {
            ...
            img.onload = () => {
              // Static IC reference images are persistent identity references,
              // not timeline-length videos. Keep their visible timeline block
              // compact (1 second) while the backend continues to use them as
              // full-generation references.
          ...
          vid.muted = true;
          vid.onerror = (e) => {
            console.error("Motion video load error", e);
            URL.revokeObjectURL(blobUrl);
            alert("Reference video preview could not be decoded. Try an MP4 with H.264 video.");
            resolve();
          };
  ...
  updateWidgetVisibility() {
    // DHan compact-layout mode: the split settings panel is the only visible
    // timing UI. Keep all legacy second/frame widgets hidden permanently.
    // Their values remain serialized and are still used by the backend.
    for (const w of [
    ...
    // Older saved workflows/frontends can recreate widget-backed timing rows.
    // Remove only unlinked duplicates; preserve any externally wired inputs.
    if (Array.isArray(this.node?.inputs)) {
      const legacyTiming = new Set([
        "start_second", "end_second", "duration_seconds",
        "start_frame", "end_frame", "duration_frames",
      ]);
      for (let i = this.node.inputs.length - 1; i >= 0; i--) {
        const input = this.node.inputs[i];
        if (legacyTiming.has(input?.name) && input.link == null) {
          this.node.removeInput(i);
        }
      }
    }
  ...
  updateUIFromSelection() {
    if (this._syncCompactH3Settings) this._syncCompactH3Settings();
    if (this.selectedSegmentIds && this.isMultiSelectActive()) {
      if (this.globalPromptInput) {
  ...
  updateRetakeUIState() {
    ...
    if (isRetake) {
      ...
      if (this.mainTrackLabel) {
        ...
        if (textSpan) textSpan.textContent = "RETAKE";
      ...
      if (this.mainTrackLabel) {
        const textSpan = this.mainTrackLabel.querySelector("span");
        if (textSpan) textSpan.textContent = "FL2VA";
  ...
  // --- Rendering logic ---
  render() {
    // H3 mode swap: normalize track geometry on every render so inherited DHan
    // callbacks cannot resurrect hidden lanes between mode switches.
    if (this._dhanH3Mode === "Ref2VA") {
      this.mainTrackEnabled = false;
      this.motionTrackEnabled = true;
      this.audioTrackEnabled = true;
      this.blockHeight = 0;
      if (this._dhanH3SharedLaneHeight) this.motionTrackHeight = this._dhanH3SharedLaneHeight;
      this.audioTrackHeight = this._dhanH3RefAudioLaneHeight || 72;
      this.canvasHeight = RULER_HEIGHT + this.motionTrackHeight + this.audioTrackHeight;
    } else if (this._dhanH3Mode === "FL2VA") {
      this.mainTrackEnabled = true;
      this.motionTrackEnabled = false;
      this.audioTrackEnabled = false;
      this.audioTrackHeight = 0;
      this.motionTrackHeight = 0;
      this.mainTrackEnabled = true;
      this.audioTrackEnabled = false;
      this.motionTrackEnabled = false;
      if (this._dhanH3SharedLaneHeight) this.blockHeight = this._dhanH3SharedLaneHeight;
      this.canvasHeight = (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight);
    }
    ...
    if (this._dhanH3Mode === "Ref2VA") {
      // H3 Ref2VA layout: REF2VA media lane first, REF AUDIO directly underneath.
      this.ctx.fillStyle = "#121212";
      this.ctx.fillRect(0, RULER_HEIGHT, width, this.motionTrackHeight);

      this.ctx.fillStyle = "#141414";
      this.ctx.fillRect(0, RULER_HEIGHT + this.motionTrackHeight, width, this.audioTrackHeight);
    } else {
      this.ctx.fillStyle = "#141414"; // Audio track bg
      this.ctx.fillRect(0, RULER_HEIGHT + this.motionTrackHeight, width, this.audioTrackHeight);

      this.ctx.fillStyle = "#121212"; // Motion track bg
      this.ctx.fillRect(0, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) + this.audioTrackHeight, width, this.motionTrackHeight);
    }
    ...
    let renderSegments = this._dhanH3Mode === "Ref2VA" ? [] : this.timeline.segments;
    let renderAudioSegments = this._dhanH3Mode === "Ref2VA" ? this.timeline.audioSegments : [];
    let renderMotionSegments = this._dhanH3Mode === "Ref2VA" ? this.timeline.motionSegments : [];

    if (this._isDragging && this._groupResizePreviewTimelines) {
      if (this._dhanH3Mode !== "Ref2VA" && this._groupResizePreviewTimelines.image) renderSegments = this._groupResizePreviewTimelines.image;
      if (this._dhanH3Mode === "Ref2VA" && this._groupResizePreviewTimelines.motion) renderMotionSegments = this._groupResizePreviewTimelines.motion;
      if (this._groupResizePreviewTimelines.audio) renderAudioSegments = this._groupResizePreviewTimelines.audio;
    } else if (this._isDragging && this._multiDragPreviewTimelines) {
      if (this._dhanH3Mode !== "Ref2VA" && this._multiDragPreviewTimelines.image) renderSegments = this._multiDragPreviewTimelines.image;
      if (this._dhanH3Mode === "Ref2VA" && this._multiDragPreviewTimelines.motion) renderMotionSegments = this._multiDragPreviewTimelines.motion;
    ...
    if (this.retakeMode) {
      ...
      const frameRate = this.getFrameRate();
      if (retakeVid) {
        // Retake uses an NLE-style sampled filmstrip rather than a single midpoint frame.
        // This keeps the whole source readable at a glance while the persistent playhead
        // indicates the exact current/scrub position.
        const baseVideoDur = retakeVid.videoDurationFrames || 0;
        const videoWidthPx = totalFrames > 0 ? (baseVideoDur / totalFrames) * width : width;
        const trackY = RULER_HEIGHT + 1;
        const trackH = this.blockHeight - 2;

        this.ctx.fillStyle = "#000";
        this.ctx.fillRect(0, trackY, videoWidthPx, trackH);

        const thumbs = Array.isArray(retakeVid.thumbnails) ? retakeVid.thumbnails : [];
        if (thumbs.length > 0 && videoWidthPx > 0 && trackH > 0) {
          // Keep the strip compact: around 8–12 visible temporal samples depending on width.
          const visibleSamples = Math.max(8, Math.min(12, Math.round(videoWidthPx / 115)));
          const tileW = videoWidthPx / visibleSamples;
          const durationSecs = baseVideoDur > 0 ? baseVideoDur / frameRate : (retakeVid.videoEl?.duration || 1);

          const nearestThumb = (targetSec) => {
            let best = thumbs[0];
            let bestDiff = Infinity;
            for (const t of thumbs) {
              const d = Math.abs((t.time || 0) - targetSec);
          ...
          for (let i = 0; i < visibleSamples; i++) {
            const sampleSec = visibleSamples <= 1 ? 0 : (i / (visibleSamples - 1)) * durationSecs;
            const t = nearestThumb(sampleSec);
            const img = t?.img;
            if (!img || !img.complete) continue;

            const natW = img.naturalWidth || img.width || 1;
            const natH = img.naturalHeight || img.height || 1;
            const srcAR = natW / Math.max(1, natH);
            const tileAR = tileW / Math.max(1, trackH);
            ...
            const dx = i * tileW;
            this.ctx.drawImage(img, sx, sy, sw, sh, dx, trackY, tileW + 0.5, trackH);

            // Subtle separators make temporal samples readable without looking like cuts.
            if (i > 0) {
              this.ctx.fillStyle = "rgba(255,255,255,0.12)";
              this.ctx.fillRect(dx, trackY, 1, trackH);
            }
          }

          // Highlight the filmstrip sample nearest the persistent playhead.
          const actualDur = Math.max(1, baseVideoDur);
          const currentNorm = Math.max(0, Math.min(1, this.currentFrame / actualDur));
          const currentIdx = Math.max(0, Math.min(visibleSamples - 1, Math.round(currentNorm * (visibleSamples - 1))));
          const hx = currentIdx * tileW;
          this.ctx.save();
          this.ctx.strokeStyle = "rgba(255,255,255,0.82)";
          this.ctx.lineWidth = 2;
          this.ctx.strokeRect(hx + 1, trackY + 1, Math.max(1, tileW - 2), Math.max(1, trackH - 2));
          this.ctx.restore();
        } else if (retakeVid.videoEl && retakeVid.videoEl.readyState >= 2 && !retakeVid.videoEl.seeking) {
          // Immediate fallback while thumbnails are still being extracted.
          const vid = retakeVid.videoEl;
          const natW = vid.videoWidth || 1;
          const natH = vid.videoHeight || 1;
          const srcAR = natW / Math.max(1, natH);
          const dstAR = videoWidthPx / Math.max(1, trackH);
          let sx = 0, sy = 0, sw = natW, sh = natH;
          if (srcAR > dstAR) {
            sw = natH * dstAR;
            sx = (natW - sw) / 2;
          ...
          this.ctx.drawImage(vid, sx, sy, sw, sh, 0, trackY, videoWidthPx, trackH);
        }
        ...
        this.ctx.fillText("Drag & Drop or Click to Add a Video", activeStart + activeWidth / 2, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) / 2);
      }
      ...
      if (this.timeline.retakeVideo) {
        ...
        this.ctx.beginPath();
        this.ctx.roundRect(rX1 - 3, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) / 2 - 20, 6, 40, 3);
        this.ctx.fill();

        this.ctx.beginPath();
        this.ctx.roundRect(rX2 - 3, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) / 2 - 20, 6, 40, 3);
        this.ctx.fill();
        ...
        // Draw "RETAKE REGION" centered label inside the retake box
        {
          ...
          const labelY = (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) - labelFontSize - labelPadY * 2 - 4;
      ...
      for (let i = 0; i < sortedSegments.length; i++) {
        ...
        if (seg.type === "ghost") {
          ...
          this.ctx.fillText("Drop to Place", startX + pxWidth / 2, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) / 2);
        } else {
        ...
        if ((seg.type === "video" || drawSource) && seg.type !== "ghost") {
          if (seg.type === "video" && pxWidth > 0) {
            ...
            if ((seg._uploading || seg._extractingThumbs) && pxWidth > 60) {
              ...
              this.ctx.fillRect(startX + 1, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) - 17, upW, 16);
              this.ctx.fillStyle = "#fff";
              this.ctx.textAlign = "center";
              this.ctx.textBaseline = "middle";
              this.ctx.fillText(upText, startX + 1 + upW / 2, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) - 9);
              this.ctx.restore();
            }
          ...
          if (seg.prompt && seg.type !== "ghost" && pxWidth > 24) {
            ...
            const overlayY = (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) - overlayH;
        ...
        // Keep text/prompt blocks readable even when they are not selected.
        if (seg.type === "text" && !isSelected) {
          this.ctx.save();
          this.ctx.strokeStyle = "rgba(210,220,230,0.42)";
          this.ctx.lineWidth = 1;
          this.ctx.strokeRect(
            startX + 0.5,
            RULER_HEIGHT + 1.5,
            Math.max(0, pxWidth - 1),
            Math.max(0, this.blockHeight - 3)
          );
          this.ctx.restore();
        }
        ...
        if (isSelected) {
          ...
          if (!this.isMultiSelectActive()) {
            ...
            this.ctx.beginPath();
            this.ctx.roundRect(startX, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) / 2 - 12, 4, 24, 2);
            this.ctx.fill();
            this.ctx.beginPath();
            this.ctx.roundRect(startX + pxWidth - 4, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) / 2 - 12, 4, 24, 2);
            this.ctx.fill();
          }
        } else {
      ...
      for (let i = 0; i < sortedMotionSegments.length; i++) {
        ...
        const pxWidth = Math.max(1, Math.floor(rawEndX) - startX);
        const isSelected = this.selectedSegmentIds.includes(seg.id);
        const trackY = this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT : RULER_HEIGHT + this.blockHeight + this.audioTrackHeight;
        ...
        if (seg.type === "ghost") {
          ...
          if (pxWidth > 0 && seg.type !== "ghost") {
            ...
            this.ctx.textAlign = "center";
            this.ctx.textBaseline = "middle";
            const label = seg.type === "text" ? "Text" : (seg.isStaticImage ? "IC-LoRA Image" : "IC-LoRA Video");
            ...
            if (seg.type !== "text" && this.node.properties.showFilenames && pxWidth > 80) {
              this.ctx.save();
              this.ctx.beginPath();
              this.ctx.rect(startX, trackY, pxWidth, this.motionTrackHeight);
              this.ctx.clip();
      ...
      for (let i = 0; i < sortedAudioSegments.length; i++) {
        ...
        const pxWidth = Math.max(1, Math.floor(rawEndX) - startX);
        const isSelected = this.selectedSegmentIds.includes(seg.id);
        const trackY = (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight);
    ...
    this.ctx.fillRect(0, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) - 1, width, 1);
    this.ctx.fillRect(0, (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) + this.audioTrackHeight - 1, width, 1);
    ...
    if (this.isMultiSelectActive() && !this.retakeMode) {
      const groupBounds = this.getMultiSelectionBounds(this._groupResizePreviewTimelines || this._multiDragPreviewTimelines || null);
      if (groupBounds) {
        const gx1 = (groupBounds.start / totalFrames) * width;
        const gx2 = (groupBounds.end / totalFrames) * width;
        const gy1 = RULER_HEIGHT + 3;
        const gy2 = (this._dhanH3Mode === "Ref2VA"
          ? RULER_HEIGHT + this.motionTrackHeight - 3
          : RULER_HEIGHT + this.blockHeight - 3);
        this.ctx.save();
        this.ctx.strokeStyle = "rgba(220,226,236,.92)";
        this.ctx.lineWidth = 1.5;
        this.ctx.setLineDash([5,4]);
        this.ctx.strokeRect(gx1, gy1, Math.max(1,gx2-gx1), Math.max(1,gy2-gy1));
        this.ctx.setLineDash([]);
        const hw=7, hh=28, hy=gy1+Math.max(0,(gy2-gy1-hh)/2);
        this.ctx.fillStyle = "rgba(225,232,242,.96)";
        this.ctx.fillRect(gx1-hw/2,hy,hw,hh);
        this.ctx.fillRect(gx2-hw/2,hy,hw,hh);
        this.ctx.restore();
      }
    }
  ...
  onMouseDown(e) {
    ...
    if (!this.retakeMode) {
      const isOverDivider = Math.abs(y - ((this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight))) <= 8;
      const isOverAudioDivider = Math.abs(y - ((this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) + this.audioTrackHeight)) <= 8;
      if (isOverDivider) {
        this._isDragging = true;
    ...
    if (this.retakeMode) {
      ...
      if (y >= RULER_HEIGHT && y <= (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight)) {
        if (!this.timeline.retakeVideo) {
          if (this.videoFileInput) {
      ...
      if (y >= RULER_HEIGHT && y <= (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight)) {
        ...
        if (this.timeline.retakeVideo && Math.abs(x - x1) <= threshold) {
          ...
          this.currentFrame = retakeStart + retakeLength;
          this._isDragging = true;
    ...
    const groupHandle = this.getMultiGroupHandleHit(x, y);
    if (groupHandle) {
      this._isDragging = true;
      this._dragType = groupHandle.dir === "left" ? "group_resize_left" : "group_resize_right";
      this._dragStartX = x;
      this._groupResizeBounds = { ...groupHandle.bounds };
      this._groupResizeInitialSegments = {
  ...
  onMouseMove(e) {
    ...
    if (this.retakeMode && !this._isDragging) {
      ...
      if (mouseY >= RULER_HEIGHT && mouseY <= (this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight)) {
        const logicalWidth = this.canvas.offsetWidth;
        const totalFrames = this.getVisualDurationFrames();
        const retakeStart = this.timeline.retakeStart ?? 0;
        const baseVideoDur = this.timeline.retakeVideo?.videoDurationFrames ?? totalFrames;
    ...
    if (!this._isDragging) {
      ...
      const isOverDivider = Math.abs(mouseY - ((this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight))) <= 8;
      const isOverAudioDivider = Math.abs(mouseY - ((this._dhanH3Mode === "Ref2VA" ? RULER_HEIGHT + this.motionTrackHeight : RULER_HEIGHT + this.blockHeight) + this.audioTrackHeight)) <= 8;
      const visibleBottom = Math.min(this.canvasHeight, this.viewport.scrollTop + this.viewport.clientHeight);
      const isAtBottom = Math.abs(mouseY - visibleBottom) <= 15;
      const viewRect = this.viewport.getBoundingClientRect();
      const isAtRightEdge = Math.abs(e.clientX - viewRect.right) <= 20;
      const groupHandle = this.getMultiGroupHandleHit(mouseX, mouseY);
    ...
    if (this.retakeMode && this._isDragging) {
      ...
      if (this._dragType === "retake_right") {
        ...
        this.timeline.retakeLength = newLength;
        this.currentFrame = this.timeline.retakeStart + newLength;
    ...
    if (this._dragType === "group_resize_left" || this._dragType === "group_resize_right") {
      this.canvas.style.cursor = "ew-resize";
      const logicalWidth = this.canvas.offsetWidth;
      const totalFrames = this.getVisualDurationFrames();
      if (!logicalWidth || totalFrames <= 0 || !this._groupResizeBounds) return;
      let mouseFrame = mouseX * (totalFrames / logicalWidth);
      const dir = this._dragType === "group_resize_left" ? "left" : "right";

      if (this.isSnapping) {
        const thresholdFrames = (15 / logicalWidth) * totalFrames;
        const candidates = [0, this.getDurationFrames(), this.getStartFrames(), this.currentFrame];
        if (this.endFramesWidget?.value !== undefined) candidates.push(parseInt(this.endFramesWidget.value, 10));
        const selected = new Set(this.selectedSegmentIds || []);
        for (const track of ["image", "motion", "audio"]) {
          for (const seg of (this._groupResizeInitialSegments?.[track] || [])) {
            if (selected.has(seg.id)) continue;
            candidates.push(seg.start, seg.start + seg.length);
          }
        }
        let best = mouseFrame, bestDiff = thresholdFrames;
        for (const c of candidates) {
          const d = Math.abs(mouseFrame - c);
          if (d < bestDiff) { bestDiff = d; best = c; }
        }
        mouseFrame = best;
      }

      this._groupResizePreviewTimelines = this.buildGroupResizePreview(mouseFrame, dir);
      this.render();
      return;
    }
  ...
  onMouseUp(e) {
    ...
    if (this._isDragging) {
      ...
      if (this._groupResizePreviewTimelines) {
        const preserveRuntime = (previewArr, originalArr) => previewArr.map(ps => {
          const orig = originalArr.find(s => s.id === ps.id);
          if (orig) {
            for (const key of ["imgObj","videoEl","thumbnails","_extractingThumbs","_uploading","_decoding","_blobUrl","_audioBuffer","waveformPeaks"]) {
              if (orig[key] !== undefined) ps[key] = orig[key];
            }
          }
          return ps;
        });
        this.timeline.segments = preserveRuntime(this._groupResizePreviewTimelines.image || [], this.timeline.segments);
        this.timeline.motionSegments = preserveRuntime(this._groupResizePreviewTimelines.motion || [], this.timeline.motionSegments);
        this.timeline.audioSegments = preserveRuntime(this._groupResizePreviewTimelines.audio || [], this.timeline.audioSegments);
        this._groupResizePreviewTimelines = null;
        this._groupResizeInitialSegments = null;
        this._groupResizeBounds = null;
      } else if (this._multiDragPreviewTimelines) {
  ...
  commitChanges(skipRender = false) {
    // Persist only the active MiniMax mode's prompt/text bank.
    // This prevents FL2VA and Ref2VA timeline prompts from converging.
    if (this._dhanH3Banks && this._dhanH3Mode) {
      const runtimeClone = (this.timeline.segments || []).map(seg => ({...seg}));
      this._dhanH3Banks[this._dhanH3Mode] = runtimeClone;

      const safeClone = runtimeClone.map(seg => {
        const out = {};
        for (const [k, v] of Object.entries(seg || {})) {
          if (k.startsWith("_")) continue;
          if (k === "imgObj" || k === "videoEl" || k === "audioEl" ||
              k === "previewThumbs" || k === "thumbnails") continue;
          if (typeof v === "function") continue;
          if (typeof HTMLElement !== "undefined" && v instanceof HTMLElement) continue;
          out[k] = v;
        }
        return out;
      });
      ...
      if (!this.node.properties) this.node.properties = {};
      if (this._dhanH3Mode === "Ref2VA") {
        this.node.properties.dhanH3Ref2vaSegments = safeClone;
      } else {
        this.node.properties.dhanH3Fl2vaSegments = safeClone;
      }

      const gp = this.globalPromptInput?.value ?? this.timeline.global_prompt ?? "";
      if (!this._dhanH3GlobalPrompts) this._dhanH3GlobalPrompts = {FL2VA:"", Ref2VA:""};
      this._dhanH3GlobalPrompts[this._dhanH3Mode] = gp;
      if (this._dhanH3Mode === "Ref2VA") {
        this.node.properties.dhanH3Ref2vaGlobalPrompt = gp;
        this.timeline.ref2va_global_prompt = gp;
      } else {
        this.node.properties.dhanH3Fl2vaGlobalPrompt = gp;
        this.timeline.fl2va_global_prompt = gp;
      }
    }
  ...
  getGapRegions() {
    ...
    // FL2VA uses the main/image lane. Ref2VA swaps that lane out entirely and
    // uses motionSegments as the visible REF2VA lane. Keep the gap '+' tied to
    // whichever lane is actually visible; otherwise the hidden FL2VA segments
    // can suppress the button and the motion '+' gets drawn below the canvas.
    if (this._dhanH3Mode === "Ref2VA") {
      let cursor = 0;
      const sortedRef = [...this.timeline.motionSegments].sort((a, b) => a.start - b.start);
      const centerY = RULER_HEIGHT + this.motionTrackHeight / 2;
      for (const seg of sortedRef) {
        if (seg.start > cursor) {
          const x0 = (cursor / totalFrames) * width;
          const x1 = (seg.start / totalFrames) * width;
          gaps.push({ track: 'motion', frameStart: cursor, frameEnd: seg.start, centerX: (x0 + x1) / 2, centerY, widthPx: x1 - x0 });
        }
      ...
      if (cursor < outputFrames) {
        ...
        gaps.push({ track: 'motion', frameStart: cursor, frameEnd: outputFrames, centerX: (x0 + x1) / 2, centerY, widthPx: x1 - x0 });
      }
    } else {
      // FL2VA image gaps
      let cursor = 0;
      ...
      // Preserve the original motion-gap behavior for non-H3/FL2VA layouts.
      cursor = 0;
    ...
    // Audio gaps. In Ref2VA the audio lane sits immediately below REF2VA.
    let cursor = 0;
    const sortedAud = [...this.timeline.audioSegments].sort((a, b) => a.start - b.start);
    const audioTop = this._dhanH3Mode === "Ref2VA"
      ? RULER_HEIGHT + this.motionTrackHeight
      : RULER_HEIGHT + this.blockHeight;
    for (const seg of sortedAud) {
      if (seg.start > cursor) {
        const x0 = (cursor / totalFrames) * width;
        const x1 = (seg.start / totalFrames) * width;
        gaps.push({ track: 'audio', frameStart: cursor, frameEnd: seg.start, centerX: (x0 + x1) / 2, centerY: audioTop + this.audioTrackHeight / 2, widthPx: x1 - x0 });
      }
      cursor = Math.max(cursor, seg.start + seg.length);
    }
    if (cursor < outputFrames) {
      const x0 = (cursor / totalFrames) * width;
      const x1 = (outputFrames / totalFrames) * width;
      gaps.push({ track: 'audio', frameStart: cursor, frameEnd: outputFrames, centerX: (x0 + x1) / 2, centerY: audioTop + this.audioTrackHeight / 2, widthPx: x1 - x0 });
    }
  ...
  _deleteRetakeVideo() {
    ...
    if (vid.videoEl) {
      // This is an intentional delete/reset, not a media decode failure.
      // Remove the error callback before clearing src; Chromium can emit an
      // error event when load() is called with an empty media source.
      vid.videoEl.onerror = null;
      vid.videoEl.onloadeddata = null;
      vid.videoEl.onloadedmetadata = null;
      vid.videoEl.onseeked = null;
      vid.videoEl.pause();
      vid.videoEl.removeAttribute("src");
      vid.videoEl.load();
    }
    if (vid._blobUrl) {
  ...
  showGapContextMenu(clientX, clientY, gap) {
    ...
    if (currentTrack === "image") {
      ...
      textBtn.onclick = () => {
        this.addSegmentInGap(gap.frameStart, gap.frameEnd, "text", gap.clickedFrame);
        this.dismissContextMenu();
      };
  ...
  showGapMenu(clientX, clientY, gap) {
    ...
    if (currentTrack === "image") {
      ...
      textBtn.addEventListener("click", () => {
        // The + button is drawn at the center of the empty gap. Center the new
        // one-second text segment on that same frame instead of placing it at
        // the far-left edge of the gap.
        const gapLength = Math.max(1, gap.frameEnd - gap.frameStart);
        const defaultLength = Math.min(Math.max(1, this.getFrameRate()), gapLength);
        const plusFrame = Math.round((gap.frameStart + gap.frameEnd) / 2);
        const preferredStart = Math.round(plusFrame - defaultLength / 2);
        this.addSegmentInGap(gap.frameStart, gap.frameEnd, "text", preferredStart);
        this.dismissGapMenu();
      });
      ...
      // In Comfyui-DHan-H3 Ref2VA this is the visible storyboard/reference lane, so its
      // gap '+' should be able to create the same text blocks as Add Text.
      if (this._dhanH3Mode === "Ref2VA") {
        const textBtn = document.createElement("button");
        textBtn.className = "pr-gap-menu-btn";
        textBtn.innerHTML = `${ICONS.text} Text Segment`;
        textBtn.addEventListener("click", () => {
          ...
          const plusFrame = Math.round((gap.frameStart + gap.frameEnd) / 2);
          const preferredStart = Math.round(plusFrame - defaultLength / 2);
          this.addRefTextSegmentInGap(gap.frameStart, gap.frameEnd, preferredStart);
          this.dismissGapMenu();
        });
        menu.appendChild(textBtn);
      }

      const refImgBtn = document.createElement("button");
      refImgBtn.className = "pr-gap-menu-btn";
      refImgBtn.innerHTML = `${ICONS.upload} Ref Image`;
      ...
      menu.appendChild(refImgBtn);

      const refVidBtn = document.createElement("button");
      refVidBtn.className = "pr-gap-menu-btn";
      refVidBtn.innerHTML = `${ICONS.motion} Ref Video`;
      refVidBtn.addEventListener("click", () => {
        this.dismissGapMenu();
        this.promptAddMotionInGap(gap.frameStart, gap.frameEnd);
      });
      menu.appendChild(refVidBtn);
    } else if (currentTrack === "audio") {
      const audBtn = document.createElement("button");
      audBtn.className = "pr-gap-menu-btn";
      audBtn.innerHTML = `${ICONS.audio} Audio Segment`;
  ...
  showSettingsMenu(anchorEl) {
    ...
    // Debug aid only. Default OFF so the Director keeps its compact height.
    const compiledPromptCtrl = document.createElement("div");
    compiledPromptCtrl.className = "pr-segmented-control";
    const cpOffSeg = document.createElement("div");
    cpOffSeg.className = "pr-segment";
    cpOffSeg.textContent = "Off";
    const cpOnSeg = document.createElement("div");
    cpOnSeg.className = "pr-segment";
    cpOnSeg.textContent = "On";

    if (this.node.properties.showCompiledPrompt === undefined) {
      this.node.properties.showCompiledPrompt = false;
    }
    const updateCompiledPromptActive = (enabled) => {
      cpOnSeg.classList.toggle("active", !!enabled);
      cpOffSeg.classList.toggle("active", !enabled);
    };
    updateCompiledPromptActive(!!this.node.properties.showCompiledPrompt);

    const setCompiledPromptVisible = (enabled) => {
      this.node.properties.showCompiledPrompt = !!enabled;
      updateCompiledPromptActive(!!enabled);
      this.node._dhanH3SetCompiledPromptVisible?.(!!enabled);
      this.commitChanges(true);
    };
    cpOffSeg.addEventListener("click", () => setCompiledPromptVisible(false));
    cpOnSeg.addEventListener("click", () => setCompiledPromptVisible(true));
    compiledPromptCtrl.appendChild(cpOnSeg);
    compiledPromptCtrl.appendChild(cpOffSeg);
    menu.appendChild(this._makeSettingRow("Compiled Prompt", compiledPromptCtrl));
  ...
  addSegmentInGap(frameStart, frameEnd, type = "text", preferredFrame = null) {
    const gapLength = Math.max(1, frameEnd - frameStart);
    const defaultLength = Math.max(1, this.getFrameRate()); // One second by default.
    const length = Math.min(defaultLength, gapLength);
    const requestedStart = preferredFrame !== null && preferredFrame !== undefined
      ? Math.round(preferredFrame)
      : Math.round(frameStart);
    const start = clamp(requestedStart, frameStart, Math.max(frameStart, frameEnd - length));
  ...
  addRefTextSegmentInGap(frameStart, frameEnd, preferredStart = null) {
    const frameRate = this.getFrameRate();
    const gapLength = Math.max(1, frameEnd - frameStart);
    const newLength = Math.min(Math.max(1, frameRate), gapLength);
    let start = preferredStart == null ? frameStart : preferredStart;
    start = Math.max(frameStart, Math.min(start, frameEnd - newLength));

    const arr = this.timeline.motionSegments || (this.timeline.motionSegments = []);
    const seg = {
      id: Date.now().toString() + Math.random().toString(36).substr(2, 5),
  ...
  addTextSegmentFreeSpace() {
    ...
    // MiniMax H3 Ref2VA has its own visible lane. Keep text prompts in that
    // lane so Add Text behaves exactly where the user is working instead of
    // adding an invisible FL2VA/main-track segment.
    if (this._dhanH3Mode === "Ref2VA") {
      const arr = this.timeline.motionSegments || (this.timeline.motionSegments = []);
...
app.registerExtension({
  name: "DHan.MiniMaxH3Director.Transplanted",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "DHanMiniMaxH3Director") {
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        // DHan compact layout: resize_method is represented by the split settings
        // panel only. Keep the native widget/input hidden across workflow restore.
        const h3ResizeMethodWidget = this.widgets?.find(w => w.name === "resize_method");
        if (h3ResizeMethodWidget) {
          if (!h3ResizeMethodWidget.value) {
            h3ResizeMethodWidget.value = "maintain aspect ratio";
          }
          hideWidget(h3ResizeMethodWidget);
          if (!h3ResizeMethodWidget._dhanOriginalType) h3ResizeMethodWidget._dhanOriginalType = h3ResizeMethodWidget.type;
          h3ResizeMethodWidget.type = "converted-widget";
          h3ResizeMethodWidget.computeSize = () => [0, -4];
          h3ResizeMethodWidget.draw = () => {};
          if (h3ResizeMethodWidget.element) h3ResizeMethodWidget.element.style.display = "none";
        }
        if (Array.isArray(this.inputs)) {
          for (let i = this.inputs.length - 1; i >= 0; i--) {
            const input = this.inputs[i];
            if (input?.name === "resize_method" && input.link == null) {
              this.removeInput(i);
            }
          }
        }
        ...
        widget.computeSize = function (width) {
          ...
          // Prefer the real DOM height. The Voice Reference panel added content
          // below the timeline, and the old fixed offset left Global Prompt
          // outside the node frame. scrollHeight keeps future panels contained too.
          const measuredHeight = Math.ceil(editor?.wrapper?.scrollHeight || editor?.container?.scrollHeight || 0);
          if (measuredHeight > 0) {
            return [Math.max(10, nodeWidth - 30), measuredHeight + 24];
          }

          const canvasH = editor ? editor.canvasHeight : CANVAS_HEIGHT;
          const propH = editor ? (editor.propHeight || 90) : 90;
          const globalPropH = editor ? (editor.globalPropHeight || 60) : 60;
          return [Math.max(10, nodeWidth - 30), canvasH + propH + globalPropH + 230];
        };
        ...
        setTimeout(() => {
          try {
            ...
            // --- MiniMax H3 cleanup layer ---
            // Keep the tuned DHan MAIN timeline behavior intact, but hide LTX-only
            // audio / IC-LoRA / voice UI that MiniMax H3 Director does not consume.
            const editor = self._timelineEditor;

            // --- Live compiled prompt preview (default hidden) ---
            // Uses the same Python planner endpoint as execution, so this is the exact
            // storyboard text DHan will hand to the H3 conditioning node.
            if (self.properties.showCompiledPrompt === undefined) {
              self.properties.showCompiledPrompt = false;
            }
            const compiledPanel = document.createElement("div");
            compiledPanel.style.cssText =
              "display:none;margin-top:8px;padding:8px 10px;border:1px solid #343434;" +
              "border-radius:6px;background:#1c1c1c;box-sizing:border-box;width:100%;";
            const compiledHead = document.createElement("div");
            compiledHead.style.cssText =
              "display:flex;align-items:center;gap:8px;margin-bottom:6px;color:#8c9aaa;" +
              "font-size:9px;font-weight:700;letter-spacing:.65px;text-transform:uppercase;";
            const compiledTitle = document.createElement("span");
            compiledTitle.textContent = "COMPILED PROMPT";
            const compiledBadge = document.createElement("span");
            compiledBadge.style.cssText =
              "margin-left:auto;color:#777;font-size:9px;font-family:ui-monospace,Consolas,monospace;";
            compiledHead.appendChild(compiledTitle);
            compiledHead.appendChild(compiledBadge);
            const compiledWarn = document.createElement("div");
            compiledWarn.style.cssText =
              "display:none;margin:0 0 6px;color:#d8a657;font-size:10px;line-height:1.35;";
            const compiledText = document.createElement("pre");
            compiledText.textContent = "…";
            compiledText.style.cssText =
              "margin:0;padding:7px 8px;max-height:190px;overflow:auto;white-space:pre-wrap;" +
              "word-break:break-word;border:1px solid #303030;border-radius:4px;background:#131313;" +
              "color:#d0d0d0;font-size:11px;line-height:1.4;font-family:ui-monospace,Consolas,monospace;";
            compiledPanel.appendChild(compiledHead);
            compiledPanel.appendChild(compiledWarn);
            compiledPanel.appendChild(compiledText);
            editor.wrapper?.appendChild(compiledPanel);
            self._dhanH3CompiledPromptPanel = compiledPanel;

            let compiledTimer = null;
            const refreshCompiledPrompt = async () => {
              if (!self.properties.showCompiledPrompt) return;
              clearTimeout(compiledTimer);
              compiledTimer = setTimeout(async () => {
                try {
                  const w = (name) => self.widgets?.find(x => x.name === name);
                  const mode = String(w("model_type")?.value || "FL2VA");
                  // Work from the editor's live timeline rather than waiting for a queue.
                  // commitChanges keeps timeline_data synced, but this explicit sync makes
                  // the preview reliable immediately after a UI edit.
                  editor.commitChanges?.(false);
                  const body = {
                    ...
                    use_custom_motion: mode === "Ref2VA",
                    use_custom_audio: mode === "Ref2VA",
                    override_audio: !!w("override_audio")?.value,
                    global_prompt: editor.timeline?.global_prompt || self.properties?.global_prompt || "",
                    extra_ref_image_count: 0,
                    ref_image_notes: "",
                  };
                  // The shared planner derives FL2VA/Ref2VA from timeline.reference_mode.
                  // Keep the preview mode synchronized with the DHan mode selector.
                  try {
                    const td = JSON.parse(body.timeline_data || "{}");
                    td.reference_mode = mode === "Ref2VA" ? "REF2VA" : "OFF";
                    td.retakeMode = mode === "Retake";
                    body.timeline_data = JSON.stringify(td);
                  } catch (_) {}
                  ...
                  compiledText.textContent = d.prompt || "";
                  compiledBadge.textContent =
                    `${d.mode || mode} · ${d.shots || 0} prompt${d.shots === 1 ? "" : "s"} · ` +
                    `${d.length || 0}f / ${d.seconds || 0}s · refs ` +
                    `${d.refs?.images || 0}i/${d.refs?.videos || 0}v/${d.refs?.audios || 0}a`;
                  compiledWarn.textContent = (d.warnings || []).join("  •  ");
                  compiledWarn.style.display = compiledWarn.textContent ? "block" : "none";
                } catch (e) {
                  compiledBadge.textContent = "preview unavailable";
                  compiledWarn.textContent = String(e?.message || e);
                  compiledWarn.style.display = "block";
                }
                editor.syncLayoutToNode?.();
                app.graph?.setDirtyCanvas?.(true, true);
              }, 250);
            };
            self._dhanH3RefreshCompiledPrompt = refreshCompiledPrompt;
            self._dhanH3SetCompiledPromptVisible = (enabled) => {
              self.properties.showCompiledPrompt = !!enabled;
              compiledPanel.style.display = enabled ? "block" : "none";
              if (enabled) refreshCompiledPrompt();
              requestAnimationFrame(() => {
                editor.syncLayoutToNode?.();
                app.graph?.setDirtyCanvas?.(true, true);
              });
            };
            self._dhanH3SetCompiledPromptVisible(!!self.properties.showCompiledPrompt);

            // Refresh after every committed timeline edit while the panel is visible.
            if (!editor._dhanCompiledPromptCommitHook && typeof editor.commitChanges === "function") {
              editor._dhanCompiledPromptCommitHook = true;
              const originalCommit = editor.commitChanges.bind(editor);
              editor.commitChanges = function() {
                const out = originalCommit(...arguments);
                if (self.properties.showCompiledPrompt) self._dhanH3RefreshCompiledPrompt?.();
                return out;
              };
            }

            // Model mismatch reminder. Director keeps its own mode selector because it
            // drives the timeline UI; Settings independently selects the lazy model branch.
            const modelWarning = document.createElement("div");
            modelWarning.style.cssText =
              "display:none;margin:4px 0 6px 0;padding:6px 9px;border-radius:4px;" +
              "background:#4a2a16;border:1px solid #b96b2c;color:#ffd2a6;" +
              "font-size:11px;font-weight:600;";
            modelWarning.textContent = "Model mismatch";
            editor.wrapper?.insertBefore(modelWarning, editor.wrapper?.firstChild || null);
            self._dhanH3ModelWarning = modelWarning;

            const syncModelMismatchWarning = () => {
              try {
                const directorMode = self.widgets?.find(w => w.name === "model_type")?.value || "FL2VA";
                const inputIndex = self.inputs?.findIndex(i => i.name === "h3_settings") ?? -1;
                const linkId = inputIndex >= 0 ? self.inputs?.[inputIndex]?.link : null;
                const link = linkId != null ? app.graph?.links?.[linkId] : null;
                const origin = link ? app.graph?.getNodeById?.(link.origin_id) : null;
                const settingsMode = origin?.widgets?.find(w => w.name === "model_select")?.value;

                if (settingsMode && settingsMode !== directorMode) {
                  modelWarning.textContent =
                    `Model mismatch — Director is ${directorMode}, Settings is ${settingsMode}. Switch one so they match.`;
            ...
            self._dhanH3ModelWarningTimer = setInterval(syncModelMismatchWarning, 250);
            const oldRemovedForModelWarning = self.onRemoved;
            self.onRemoved = function() {
              if (self._dhanH3ModelWarningTimer) clearInterval(self._dhanH3ModelWarningTimer);
              oldRemovedForModelWarning?.apply(this, arguments);
            };

            // MiniMax mode-isolated prompt/timeline banks.
            // Runtime banks preserve rich segment objects while node.properties keeps
            // JSON-safe copies for workflow save/load.
            const cloneSegmentsRuntime = (segments) =>
              (segments || []).map(seg => ({...seg}));

            const cloneSegmentsForSave = (segments) =>
              (segments || []).map(seg => {
                const out = {};
                for (const [k, v] of Object.entries(seg || {})) {
                  if (k.startsWith("_")) continue;
            ...
            const savedFl = Array.isArray(self.properties.dhanH3Fl2vaSegments)
              ? self.properties.dhanH3Fl2vaSegments : null;
            const savedRef = Array.isArray(self.properties.dhanH3Ref2vaSegments)
              ? self.properties.dhanH3Ref2vaSegments : null;

            editor._dhanH3Banks = {
              FL2VA: cloneSegmentsRuntime(savedFl || editor.timeline.segments || []),
              Ref2VA: cloneSegmentsRuntime(savedRef || []),
            };

            editor._dhanH3GlobalPrompts = {
              FL2VA: self.properties.dhanH3Fl2vaGlobalPrompt ??
                editor.timeline.fl2va_global_prompt ??
                editor.timeline.global_prompt ?? "",
              Ref2VA: self.properties.dhanH3Ref2vaGlobalPrompt ??
                editor.timeline.ref2va_global_prompt ?? "",
            };

            const saveActiveModeState = () => {
              const mode = editor._dhanH3Mode || "FL2VA";
              const segs = cloneSegmentsRuntime(editor.timeline.segments || []);
              editor._dhanH3Banks[mode] = segs;

              if (mode === "Ref2VA") {
                self.properties.dhanH3Ref2vaSegments = cloneSegmentsForSave(segs);
              } else {
                self.properties.dhanH3Fl2vaSegments = cloneSegmentsForSave(segs);
              }

              const gp = editor.globalPromptInput?.value ?? editor.timeline.global_prompt ?? "";
              editor._dhanH3GlobalPrompts[mode] = gp;
              if (mode === "Ref2VA") {
                self.properties.dhanH3Ref2vaGlobalPrompt = gp;
                editor.timeline.ref2va_global_prompt = gp;
              } else {
                self.properties.dhanH3Fl2vaGlobalPrompt = gp;
                editor.timeline.fl2va_global_prompt = gp;
              }
            };

            const loadModeState = (mode) => {
              editor.timeline.segments = cloneSegmentsRuntime(editor._dhanH3Banks[mode] || []);
              editor.loadMedia?.();

              const gp = editor._dhanH3GlobalPrompts[mode] || "";
              editor.timeline.global_prompt = gp;
              if (editor.globalPromptInput) editor.globalPromptInput.value = gp;
              const gpWidget = self.widgets?.find(w => w.name === "global_prompt");
              if (gpWidget) gpWidget.value = gp;

              editor.selectedIndex = -1;
              editor.selectedSegmentIds = [];
              editor.selectionType = mode === "Ref2VA" ? "motion" : "image";
              editor._ghostSegmentId = null;
              editor._ghostTrack = null;
              editor._previewSegments = null;
              editor._previewSiblingSegments = null;
              editor.updateUIFromSelection?.();
            };


            // --- H3 single-lane behavior overrides ---
            // The transplanted DHan editor assumes 3 simultaneous tracks. H3 does not.
            // Filter all gap/add logic to the currently active MiniMax lane only.
            if (!editor._dhanH3OriginalGetGapRegions) {
              editor._dhanH3OriginalGetGapRegions = editor.getGapRegions.bind(editor);
              editor.getGapRegions = function() {
                const all = this._dhanH3OriginalGetGapRegions();
                if (this._dhanH3Mode === "Ref2VA") {
                  return all.filter(g => g.track === "motion" || g.track === "audio");
                }
                return all.filter(g => g.track === "image");
              };
            }

            // Single-lane track hit testing.
            if (!editor._dhanH3OriginalGetTrackFromY) {
              editor._dhanH3OriginalGetTrackFromY = editor.getTrackFromY.bind(editor);
              editor.getTrackFromY = function(y) {
                if (this._dhanH3Mode !== "Ref2VA") return "image";
                const rel = y - RULER_HEIGHT;
                if (rel < this.motionTrackHeight) return "motion";
                if (rel < this.motionTrackHeight + this.audioTrackHeight) return "audio";
                return "motion";
              };
            }

            // Sidebar heights must follow the one visible H3 lane rather than
            // restoring hidden DHan audio/motion heights.
            if (!editor._dhanH3OriginalUpdateSidebarHeights) {
              editor._dhanH3OriginalUpdateSidebarHeights = editor.updateSidebarHeights.bind(editor);
              editor.updateSidebarHeights = function() {
                const isRef = this._dhanH3Mode === "Ref2VA";
                const isRetake = this._dhanH3Mode === "Retake" || this.retakeMode === true;
                setH3TrackLabel(this.mainTrackLabel, isRetake ? "RETAKE" : "FL2VA");
                setH3TrackLabel(this.motionTrackLabel, "REF2VA");
                this.mainTrackEnabled = !isRef;
                this.motionTrackEnabled = isRef;
                this.audioTrackEnabled = false;
                if (this.mainTrackLabel) {
                  this.mainTrackLabel.style.height = isRef ? "0px" : `${this.blockHeight}px`;
                  this.mainTrackLabel.style.display = isRef ? "none" : "flex";
                }
                if (this.motionTrackLabel) {
                  this.motionTrackLabel.style.height = isRef ? `${this.motionTrackHeight}px` : "0px";
                  this.motionTrackLabel.style.display = isRef ? "flex" : "none";
                }
                if (this.audioTrackLabel) {
                  this.audioTrackLabel.style.height = isRef ? `${this.audioTrackHeight}px` : "0px";
                  this.audioTrackLabel.style.display = isRef ? "flex" : "none";
                }
              };
            }

            // In H3 single-lane mode, disable DHan's inter-track divider resize
            // cursors/drag states. The prompt/global-prompt panel resize handles
            // remain untouched.
            if (!editor._dhanH3OriginalOnMouseMove) {
              editor._dhanH3OriginalOnMouseMove = editor.onMouseMove.bind(editor);
              editor.onMouseMove = function(e) {
                if (this._dhanH3Mode) {
                  const pos = this.getMousePos(e);
                  const laneBottom = RULER_HEIGHT +
                    (this._dhanH3Mode === "Ref2VA" ? this.motionTrackHeight : this.blockHeight);

                  // Do not let hidden-track dividers activate.
                  if (!this._isDragging &&
                      Math.abs(pos.y - laneBottom) <= 10 &&
                      pos.y < this.canvasHeight + 2) {
                    this.canvas.style.cursor = "default";
                  }

                  // Cancel stale DHan divider drag modes if a mode switch left one armed.
                  if (["divider", "audio_divider", "height_resize"].includes(this._dragType)) {
                    this._isDragging = false;
                    this._dragType = null;
                    document.body.style.userSelect = "";
                    document.body.style.cursor = "";
                ...
                return this._dhanH3OriginalOnMouseMove(e);
              };
            }

            if (!editor._dhanH3OriginalOnMouseDown) {
              editor._dhanH3OriginalOnMouseDown = editor.onMouseDown.bind(editor);
              editor.onMouseDown = function(e) {
                if (this._dhanH3Mode) {
                  const p = this.getMousePos(e);
                  const laneBottom = RULER_HEIGHT +
                    (this._dhanH3Mode === "Ref2VA" ? this.motionTrackHeight : this.blockHeight);
                  // Suppress old divider hit-zone at the bottom of the only lane.
                  if (Math.abs(p.y - laneBottom) <= 8 && p.y >= RULER_HEIGHT) {
                    // Let ordinary segment interactions through, but don't start divider resize.
                    const hit = this.getHitTest?.(p.x, p.y);
                    if (!hit) {
                      e.preventDefault();
                      e.stopPropagation();
                      return;
                ...
                return this._dhanH3OriginalOnMouseDown(e);
              };
            }


            // H3 mode determines the only active lane; inherited DHan eye toggles are misleading.
            if (editor.mainTrackLabel) {
              editor.mainTrackLabel.querySelectorAll("button, .track-toggle, .pr-track-toggle").forEach(el => {
                el.style.display = "none";
              });
            }
            if (editor.motionTrackLabel) {
              editor.motionTrackLabel.querySelectorAll("button, .track-toggle, .pr-track-toggle").forEach(el => {
                el.style.display = "none";
              });
            }

            // MiniMax H3 sizing controls: restore the tuned DHan 0=AUTO behavior
            // even when an older serialized widget carried min=32 metadata.
            const fixAutoDimensionWidget = (name, fallback) => {
              ...
              w.options.max = 8192;
              w.options.step = 1;
              if (w.value === undefined || w.value === null || Number.isNaN(Number(w.value))) {
                w.value = fallback;
              }
              // Do not auto-convert 0 to 32; 0 is intentionally AUTO.
              if (Number(w.value) < 0) w.value = 0;
            };
            fixAutoDimensionWidget("custom_width", 0);
            fixAutoDimensionWidget("custom_height", 0);

            const resizeMethodWidget = self.widgets?.find(x => x.name === "resize_method");
            if (resizeMethodWidget && (!resizeMethodWidget.value || resizeMethodWidget.value === "crop")) {
              // New H3 default mirrors DHan's aspect-preserving AUTO dimension workflow.
              resizeMethodWidget.value = "maintain aspect ratio";
            }

            const hideButtonByText = (text) => {
              const buttons = [...editor.wrapper.querySelectorAll("button")];
            ...
            hideButtonByText("Add Video");

            // H3 uses one shared lane height in either model mode.
            const h3InitialLaneHeight = Math.max(
              80,
              Number(self.properties?.dhanH3LaneHeight || editor.blockHeight || 100)
            );
            if (editor.audioTrackLabel) editor.audioTrackLabel.style.display = "none";
            editor.audioTrackEnabled = false;
            editor.audioTrackHeight = 0;

            // The inherited secondary-track "Audio: OFF" button is irrelevant to H3.
            if (editor.motionTrackLabel) {
              for (const btn of editor.motionTrackLabel.querySelectorAll("button")) {
                btn.style.display = "none";
              }
            }
            if (editor.audioTrackLabel) {
              for (const btn of editor.audioTrackLabel.querySelectorAll("button")) {
                btn.style.display = "none";
              }
            }

            // Hide any persistent Voice Ref panel inherited from DHan/LTX.
            for (const child of [...editor.wrapper.children]) {
              const text = (child.textContent || "").trim();
              if (text.startsWith("VOICE REF")) {
                child.style.display = "none";
              }
            }

            const hideTrackRows = () => {
              // AUDIO is removed entirely. REF VIDEO is mode-dependent below.
              if (editor.audioTrackLabel) editor.audioTrackLabel.style.display = "none";
              editor.audioTrackHeight = 0;
              editor.root?.querySelectorAll?.("button").forEach(btn => {
                if ((btn.textContent || "").trim().includes("Add Audio")) btn.style.display = "none";
              });
              editor.updateSidebarHeights?.();
            };

            // Ref2VA subject references live directly in the Director.
            // Keep them completely outside the timeline interaction code: the panel only
            // edits timeline.characters, which minimax_plan already understands natively.
            const REF_SUBJECTS_DEFAULT = 3;
            const REF_SUBJECTS_MIN = 1;
            const REF_SUBJECTS_MAX = 9;

            const ensureRefSubjects = () => {
              if (!Array.isArray(editor.timeline.characters) || !editor.timeline.characters.length) {
                editor.timeline.characters = Array.from(
                  { length: REF_SUBJECTS_DEFAULT },
                  () => ({ images: [], description: "" })
                );
              }
              const count = Math.max(
                REF_SUBJECTS_MIN,
                Math.min(REF_SUBJECTS_MAX, editor.timeline.characters.length)
              );
              editor.timeline.characters = editor.timeline.characters.slice(0, count).map(c => ({
                // Match the reference Director: each subject can carry up to two views.
                images: Array.isArray(c?.images) ? c.images.slice(0, 2) : [],
                description: c?.description || ""
              }));
              while (editor.timeline.characters.length < count) {
                editor.timeline.characters.push({ images: [], description: "" });
              }
              return editor.timeline.characters;
            };

            const refImageSrc = (imgData) => {
              if (!imgData) return "";
              if (imgData.b64) return imgData.b64;
              if (imgData.name) {
                const parts = String(imgData.name).split("/");
                const fn = parts.pop();
                const sf = parts.join("/");
            ...
            const storeRefImage = async (file) => {
              ...
              try {
                ...
                const upName = `dhanh3ref_${base}_${Date.now()}${ext}`;
                const body = new FormData();
                body.append("image", new File([file], upName, { type: file.type || "image/png" }));
                body.append("subfolder", "whatdreamscost");
                const resp = await api.fetchApi("/upload/image", { method: "POST", body });
                if (resp.status === 200) {
                  const data = await resp.json();
                  const sf = data.subfolder || "";
                  return { name: sf ? `${sf}/${data.name}` : data.name };
                }
              } catch (err) {
                console.warn("[Comfyui-DHan-H3] Ref2VA subject upload failed; using embedded fallback.", err);
              }

              return await new Promise((resolve) => {
                const reader = new FileReader();
                reader.onload = () => resolve({ b64: reader.result, name: file.name || "reference" });
                reader.onerror = () => resolve(null);
                reader.readAsDataURL(file);
              });
            };

            const storeRefVoice = async (file) => {
              if (!file || !(file.type?.startsWith("audio/") || file.type?.startsWith("video/"))) return null;
              try {
                const safe = (file.name || "voice_ref").replace(/[^a-zA-Z0-9_.-]/g, "_");
                const body = new FormData();
                body.append("image", new File([file], `dhanh3voice_${Date.now()}_${safe}`, { type: file.type || "audio/wav" }));
                body.append("type", "input");
                body.append("subfolder", "whatdreamscost");
                body.append("overwrite", "true");
                const resp = await api.fetchApi("/upload/image", { method: "POST", body });
                ...
                const data = await resp.json();
                const sf = data.subfolder || "";
                return { name: sf ? `${sf}/${data.name}` : data.name, displayName: file.name || data.name };
              } catch (err) {
                console.error("[Comfyui-DHan-H3] Voice reference upload failed", err);
                alert(`Voice reference upload failed: ${err.message || err}`);
                return null;
              }
            };
            ...
            refPanel.className = "dhan-h3-ref-subject-panel";
            refPanel.style.cssText =
              "display:none;margin:0 0 7px 0;padding:7px 8px;background:#1d1d1d;" +
              "border:1px solid #111;border-radius:5px;box-sizing:border-box;min-width:0;";
            ...
            const refHeadTitle = document.createElement("span");
            refHeadTitle.textContent = "Ref2VA Subjects";
            refHeadTitle.style.cssText =
              "color:#7f91a7;font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;";

            const refStepper = document.createElement("div");
            refStepper.style.cssText = "display:flex;align-items:center;gap:5px;flex:0 0 auto;";
            const refStepMinus = document.createElement("button");
            const refStepCount = document.createElement("span");
            const refStepPlus = document.createElement("button");
            for (const b of [refStepMinus, refStepPlus]) {
              b.type = "button";
              b.style.cssText =
                "width:22px;height:22px;min-width:22px;padding:0;border:1px solid #3c3c3c;" +
                "border-radius:4px;background:#242424;color:#aaa;font:13px/20px sans-serif;cursor:pointer;";
            }
            refStepMinus.textContent = "−";
            refStepMinus.title = "Remove the last subject";
            refStepPlus.textContent = "+";
            refStepPlus.title = "Add another subject";
            refStepCount.style.cssText =
              "min-width:68px;text-align:center;color:#777;font-size:9px;text-transform:uppercase;";
            refStepper.append(refStepMinus, refStepCount, refStepPlus);
            refHead.append(refHeadTitle, refStepper);
            refPanel.appendChild(refHead);

            const refCards = document.createElement("div");
            refCards.style.cssText = "display:grid;gap:7px;min-width:0;";
            refPanel.appendChild(refCards);

            // Voice references are a separate Ref2VA reference pool, not properties of
            // individual @char subjects. H3 accepts up to three audio references total;
            // the planner combines these slots with the timed REF AUDIO lane and caps the
            // final native ref_audios list at that limit.
            const ensureVoiceRefs = () => {
              const src = Array.isArray(editor.timeline.voiceReferences) ? editor.timeline.voiceReferences : [];
              editor.timeline.voiceReferences = Array.from({ length: 3 }, (_, i) => src[i] || null);
              return editor.timeline.voiceReferences;
            };

            const voiceSection = document.createElement("div");
            voiceSection.style.cssText =
              "margin-top:8px;padding-top:7px;border-top:1px solid #303030;min-width:0;";
            const voiceHead = document.createElement("div");
            voiceHead.style.cssText =
              "display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px;";
            const voiceTitle = document.createElement("span");
            voiceTitle.textContent = "Voice References";
            voiceTitle.style.cssText =
              "color:#7f91a7;font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;";
            const voiceUsage = document.createElement("span");
            voiceUsage.style.cssText = "color:#666;font-size:9px;text-transform:uppercase;";
            voiceHead.append(voiceTitle, voiceUsage);
            voiceSection.appendChild(voiceHead);
            const voiceGrid = document.createElement("div");
            voiceGrid.style.cssText = "display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;min-width:0;";
            voiceSection.appendChild(voiceGrid);
            refPanel.appendChild(voiceSection);

            const renderVoiceRefs = () => {
              const refs = ensureVoiceRefs();
              voiceGrid.innerHTML = "";
              const timedAudioCount = (editor.timeline.audioSegments || []).filter(s => s?.audioFile || s?.audioB64).length;
              const voiceCount = refs.filter(v => v?.name).length;
              voiceUsage.textContent = `${Math.min(3, voiceCount + timedAudioCount)} / 3 AUDIO REFS USED`;

              refs.forEach((vr, idx) => {
                const slot = document.createElement("button");
                slot.type = "button";
                slot.title = `Voice ${idx + 1} reference`;
                slot.style.cssText =
                  "height:34px;min-width:0;box-sizing:border-box;border:1px dashed #474747;border-radius:4px;" +
                  "background:#181818;color:#777;font-size:9px;cursor:pointer;white-space:nowrap;overflow:hidden;" +
                  "text-overflow:ellipsis;padding:0 8px;text-align:left;";
                const refreshLabel = () => {
                  const cur = ensureVoiceRefs()[idx];
                  slot.textContent = cur?.name
                    ? `Voice ${idx + 1}: ${cur.displayName || String(cur.name).split("/").pop()}   ×`
                    : `Drop / Add Voice ${idx + 1}`;
                  slot.style.borderStyle = cur?.name ? "solid" : "dashed";
                  slot.style.color = cur?.name ? "#aaa" : "#777";
                };
                refreshLabel();

                const setVoiceFile = async (file) => {
                  if (!file) return;
                  const refsNow = ensureVoiceRefs();
                  const currentFilled = refsNow.filter(v => v?.name).length;
                  const timedNow = (editor.timeline.audioSegments || []).filter(s => s?.audioFile || s?.audioB64).length;
                  if (!refsNow[idx]?.name && currentFilled + timedNow >= 3) {
                    alert("MiniMax H3 Ref2VA supports up to 3 audio reference clips total, including the REF AUDIO lane.");
                    return;
                  }
                  const stored = await storeRefVoice(file);
                  if (stored) refsNow[idx] = stored;
                  renderVoiceRefs();
                ...
                slot.addEventListener("click", (e) => {
                  ...
                  fi.addEventListener("change", async (ev) => setVoiceFile(ev.target.files?.[0]));
                  fi.click();
                });
                slot.addEventListener("dragover", (e) => {
                  e.preventDefault(); e.stopPropagation(); slot.style.borderColor = "#888";
                });
                slot.addEventListener("dragleave", (e) => {
                  e.stopPropagation(); slot.style.borderColor = "#474747";
                });
                slot.addEventListener("drop", async (e) => {
                  e.preventDefault(); e.stopPropagation(); slot.style.borderColor = "#474747";
                  const file = Array.from(e.dataTransfer?.files || []).find(
                    f => f.type?.startsWith("audio/") || f.type?.startsWith("video/")
                  );
                  await setVoiceFile(file);
                });
                voiceGrid.appendChild(slot);
              });
            };

            const renderRefSubjects = () => {
              const chars = ensureRefSubjects();
              refCards.innerHTML = "";
              refStepCount.textContent = `${chars.length} SUBJECT${chars.length === 1 ? "" : "S"}`;
              refStepMinus.disabled = chars.length <= REF_SUBJECTS_MIN;
              refStepPlus.disabled = chars.length >= REF_SUBJECTS_MAX;
              refStepMinus.style.opacity = refStepMinus.disabled ? ".35" : "1";
              refStepPlus.style.opacity = refStepPlus.disabled ? ".35" : "1";

              // Four cards across matches the reference layout while keeping added subjects compact.
              const cols = Math.max(1, Math.min(4, chars.length));
              refCards.style.gridTemplateColumns = `repeat(${cols}, minmax(0, 1fr))`;

              chars.forEach((data, idx) => {
                const card = document.createElement("div");
                card.style.cssText =
                  "min-width:0;padding:6px;background:#202020;border:1px dashed #3f3f3f;" +
                ...
                title.textContent = `@char${idx + 1}`;
                title.style.cssText = "font-size:10px;font-weight:700;color:#999;";
                const count = document.createElement("span");
                count.textContent = `${data.images.length}/2`;
                count.style.cssText = "font-size:9px;color:#606060;";
                top.append(title, count);
                card.appendChild(top);

                const imageRow = document.createElement("div");
                imageRow.style.cssText =
                  "height:58px;display:flex;gap:4px;align-items:stretch;margin-bottom:5px;";

                const addImage = document.createElement("button");
                addImage.type = "button";
                addImage.title = "Add Ref2VA subject image";
                addImage.style.cssText =
                  "flex:1;min-width:0;border:1px dashed #555;border-radius:3px;background:#181818;" +
                  "color:#777;font-size:10px;cursor:pointer;overflow:hidden;padding:0;";
                addImage.textContent = data.images.length ? "+" : "Drop / Add Image";
                ...
                const chooseFiles = () => {
                  const fi = document.createElement("input");
                    ...
                    for (const file of files) {
                      if (editor.timeline.characters[idx].images.length >= 2) break;
                      const stored = await storeRefImage(file);
                      if (stored) editor.timeline.characters[idx].images.push(stored);
                    }
                    renderRefSubjects();
                ...
                addImage.addEventListener("click", (e) => { e.stopPropagation(); chooseFiles(); });
                addImage.addEventListener("dragover", (e) => {
                  e.preventDefault(); e.stopPropagation(); addImage.style.borderColor = "#888";
                });
                addImage.addEventListener("dragleave", (e) => {
                  e.stopPropagation(); addImage.style.borderColor = "#555";
                });
                addImage.addEventListener("drop", async (e) => {
                  e.preventDefault(); e.stopPropagation(); addImage.style.borderColor = "#555";
                ...
                data.images.forEach((imgData, imgIdx) => {
                  ...
                  wrap.style.cssText =
                    "position:relative;flex:1;min-width:0;height:58px;border-radius:3px;" +
                    "overflow:hidden;background:#151515;border:1px solid #333;";
                  const im = document.createElement("img");
                  im.src = refImageSrc(imgData);
                  ...
                  del.style.cssText =
                    "position:absolute;right:2px;top:2px;width:18px;height:18px;line-height:15px;" +
                    "padding:0;border:0;border-radius:9px;background:rgba(0,0,0,.72);color:#ddd;" +
                    "font-size:14px;cursor:pointer;";
                  del.addEventListener("click", (e) => {
                    e.stopPropagation();
                    editor.timeline.characters[idx].images.splice(imgIdx, 1);
                    renderRefSubjects();
                    editor.commitChanges();
                  });
                  wrap.append(im, del);
                ...
                if (data.images.length < 2) imageRow.appendChild(addImage);
                card.appendChild(imageRow);

                const desc = document.createElement("textarea");
                desc.value = data.description || "";
                desc.placeholder = "manual description...";
                desc.rows = 2;
                desc.style.cssText =
                  "width:100%;height:38px;min-height:38px;resize:none;box-sizing:border-box;" +
                  "border:1px solid #333;border-radius:3px;background:#171717;color:#bbb;" +
                  "font:10px sans-serif;padding:4px;outline:none;";
                desc.addEventListener("input", () => {
                  editor.timeline.characters[idx].description = desc.value;
                  editor.commitChanges(true);
                });
                desc.addEventListener("click", (e) => e.stopPropagation());
                card.appendChild(desc);

                refCards.appendChild(card);
              });
            };

            refStepMinus.addEventListener("click", (e) => {
              e.stopPropagation();
              const chars = ensureRefSubjects();
              if (chars.length <= REF_SUBJECTS_MIN) return;
              editor.timeline.characters.splice(chars.length - 1, 1);
              renderRefSubjects();
              editor.commitChanges(true);
            });
            refStepPlus.addEventListener("click", (e) => {
              e.stopPropagation();
              const chars = ensureRefSubjects();
              if (chars.length >= REF_SUBJECTS_MAX) return;
              editor.timeline.characters.push({ images: [], description: "" });
              renderRefSubjects();
              editor.commitChanges(true);
            });
            ...
            renderRefSubjects();
            renderVoiceRefs();
            // Ref2VA subjects are supporting identity/reference data, so keep them below
            // the complete timeline/editor instead of above it. appendChild intentionally
            // places the panel after the timeline, playback controls and prompt editors.
            editor.wrapper.appendChild(refPanel);
            self._dhanH3RefPanel = refPanel;
            self._dhanH3RenderRefSubjects = renderRefSubjects;
            self._dhanH3RenderVoiceRefs = renderVoiceRefs;

            const setH3TrackLabel = (el, text) => {
              if (!el) return;
              // createTrackLabel's first span is the visible title span.
              const spans = [...el.querySelectorAll("span")];
              const title = spans.find(s => {
                const t = (s.textContent || "").trim();
                return t === "MAIN" || t === "FL2VA" || t === "REF" ||
                       t === "REF VIDEO" || t === "REF2VA" || t === "VIDEO";
              }) || spans[0];
              if (title) title.textContent = text;
            };

            const modelTypeWidget = self.widgets?.find(w => w.name === "model_type");
            editor._dhanH3Mode = modelTypeWidget?.value === "Ref2VA" ? "Ref2VA" :
                                (modelTypeWidget?.value === "Retake" ? "Retake" : "FL2VA");
            if (editor._dhanH3Mode !== "Retake") loadModeState(editor._dhanH3Mode);
            editor._dhanH3SharedLaneHeight = h3InitialLaneHeight;
            editor._dhanH3MainLaneHeight = h3InitialLaneHeight;
            editor._dhanH3RefLaneHeight = h3InitialLaneHeight;
            editor._dhanH3RefAudioLaneHeight = Number(self.properties?.dhanH3RefAudioLaneHeight || 72);

            // Safe H3 single-lane vertical resize, shared by FL2VA and Ref2VA.
            if (!editor._dhanH3LaneResizeInstalled && editor.canvas) {
              editor._dhanH3LaneResizeInstalled = true;
              editor._dhanH3LaneResizing = false;

              const applyH3LaneHeight = (value) => {
                const h = Math.max(80, Math.min(520, Math.round(value)));
                editor._dhanH3SharedLaneHeight = h;
                editor._dhanH3MainLaneHeight = h;
                editor._dhanH3RefLaneHeight = h;
                if (!self.properties) self.properties = {};
                self.properties.dhanH3LaneHeight = h;

                if (editor._dhanH3Mode === "Ref2VA") {
                  editor.blockHeight = 0;
                  editor.motionTrackHeight = h;
                } else {
                  editor.blockHeight = h;
                  editor.motionTrackHeight = 0;
                }
                if (editor._dhanH3Mode === "Ref2VA") {
                  editor.audioTrackHeight = editor._dhanH3RefAudioLaneHeight || 72;
                } else {
                  editor.audioTrackHeight = 0;
                }
                editor.canvasHeight = RULER_HEIGHT + h + editor.audioTrackHeight;

                const dpr = window.devicePixelRatio || 1;
                const logicalWidth = editor.canvas.offsetWidth || editor._lastWidth || 1;
                editor.canvas.width = Math.max(1, Math.round(logicalWidth * dpr));
                editor.canvas.height = Math.max(1, Math.round(editor.canvasHeight * dpr));
                editor.canvas.style.height = `${editor.canvasHeight}px`;
              ...
              editor._dhanH3ApplyLaneHeight = applyH3LaneHeight;

              editor.canvas.addEventListener("mousedown", (e) => {
                if (!editor._dhanH3Mode) return;
                const p = editor.getMousePos(e);
                const bottom = RULER_HEIGHT + editor._dhanH3SharedLaneHeight;
                if (Math.abs(p.y - bottom) > 10) return;

                editor._dhanH3LaneResizing = true;
                editor._dhanH3LaneResizeStartY = e.clientY;
                editor._dhanH3LaneResizeStartH = editor._dhanH3SharedLaneHeight;
                document.body.style.userSelect = "none";
                document.body.style.cursor = "ns-resize";
                e.preventDefault();
                e.stopImmediatePropagation();
              ...
              window.addEventListener("mousemove", (e) => {
                if (!editor._dhanH3LaneResizing) return;
                const delta = e.clientY - editor._dhanH3LaneResizeStartY;
                applyH3LaneHeight(editor._dhanH3LaneResizeStartH + delta);
                e.preventDefault();
              }, true);
              ...
              editor.canvas.addEventListener("mousemove", (e) => {
                ...
                const p = editor.getMousePos(e);
                const bottom = RULER_HEIGHT + editor._dhanH3SharedLaneHeight;
                if (Math.abs(p.y - bottom) <= 10) {
                  editor.canvas.style.cursor = "ns-resize";
                }
              }, true);
            }
            ...
            const syncRefStatus = () => {
              const rawMode = String(modelTypeWidget?.value || "FL2VA");
              const isRetake = rawMode === "Retake";
              const isRef = rawMode === "Ref2VA";
              const nextMode = isRetake ? "Retake" : (isRef ? "Ref2VA" : "FL2VA");
              const previousMode = editor._dhanH3Mode || "FL2VA";

              if (previousMode !== nextMode) {
                if (previousMode !== "Retake") saveActiveModeState();
                if (nextMode !== "Retake") loadModeState(nextMode);
              }
              editor._dhanH3Mode = nextMode;
              editor.retakeMode = isRetake;
              editor.timeline.retakeMode = isRetake;
              if (self.properties) self.properties.retakeMode = isRetake;

              refPanel.style.display = isRef ? "block" : "none";
              if (isRef) renderRefSubjects();

              if (isRetake) {
                // Reuse the mature inherited retake filmstrip/range editor, but expose it
                // through the H3 Mode selector instead of the old hidden BETA toggle.
                editor.updateRetakeUIState?.();
                setH3TrackLabel(editor.mainTrackLabel, "RETAKE");
                if (editor.uploadVideoBtn) editor.uploadVideoBtn.style.display = "";
                if (editor.uploadRefImageBtn) editor.uploadRefImageBtn.style.display = "none";
                if (editor.uploadMotionBtn) editor.uploadMotionBtn.style.display = "none";
                if (editor.uploadAudioBtn) editor.uploadAudioBtn.style.display = "none";
                if (editor.addAudioBtn) editor.addAudioBtn.style.display = "none";
                if (editor.mainTrackLabel) editor.mainTrackLabel.style.display = "flex";
              } else if (isRef) {
                setH3TrackLabel(editor.motionTrackLabel, "REF2VA");
                // True lane swap: FL2VA disappears and REF2VA takes its place.
                editor.mainTrackEnabled = false;
                editor.blockHeight = 0;
                if (editor.mainTrackLabel) editor.mainTrackLabel.style.display = "none";

                editor.motionTrackEnabled = true;
                editor.motionTrackHeight = editor._dhanH3SharedLaneHeight;
                if (editor.motionTrackLabel) editor.motionTrackLabel.style.display = "flex";

                if (editor.uploadBtn) editor.uploadBtn.style.display = "none";
                if (editor.addTextBtn) editor.addTextBtn.style.display = "";
                if (editor.uploadRefImageBtn) editor.uploadRefImageBtn.style.display = "";
                if (editor.uploadMotionBtn) editor.uploadMotionBtn.style.display = "";
                if (editor.uploadAudioBtn) editor.uploadAudioBtn.style.display = "";
                if (editor.addAudioBtn) editor.addAudioBtn.style.display = "none";

                editor.audioTrackEnabled = true;
                editor.audioTrackHeight = editor._dhanH3RefAudioLaneHeight;
                if (editor.audioTrackLabel) editor.audioTrackLabel.style.display = "flex";
              } else {
                setH3TrackLabel(editor.mainTrackLabel, "FL2VA");
                // FL2VA owns the single visible lane.
                editor.mainTrackEnabled = true;
                editor.blockHeight = editor._dhanH3SharedLaneHeight;
                if (editor.mainTrackLabel) editor.mainTrackLabel.style.display = "flex";

                editor.motionTrackEnabled = false;
                editor.motionTrackHeight = 0;
                if (editor.motionTrackLabel) editor.motionTrackLabel.style.display = "none";

                if (editor.uploadBtn) editor.uploadBtn.style.display = "";
                if (editor.addTextBtn) editor.addTextBtn.style.display = "";
                if (editor.uploadRefImageBtn) editor.uploadRefImageBtn.style.display = "none";
                if (editor.uploadMotionBtn) editor.uploadMotionBtn.style.display = "none";
                if (editor.uploadAudioBtn) editor.uploadAudioBtn.style.display = "none";
              ...
              // Standalone REF AUDIO exists only in Ref2VA.
              if (!isRef) {
                editor.audioTrackEnabled = false;
                editor.audioTrackHeight = 0;
                if (editor.audioTrackLabel) editor.audioTrackLabel.style.display = "none";
              ...
              if (!isRetake) {
                editor.canvasHeight =
                  RULER_HEIGHT +
                  (isRef
                    ? editor.motionTrackHeight + editor.audioTrackHeight
                    : editor.blockHeight);
              }

              editor.selectedIndex = -1;
              editor.selectedSegmentIds = [];
              editor._hoveredGapIdx = -1;
              editor._ghostSegmentId = null;
              editor._ghostTrack = null;
              ...
              if (editor.canvas) {
                ...
                if (editor.ctx) {
                  editor.ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
                  editor.ctx.clearRect(0, 0, logicalWidth, editor.canvasHeight);
                }
              }
              ...
              requestAnimationFrame(() => {
                ...
                if (size) self.setSize?.([self.size?.[0] || size[0], size[1]]);
                app.graph?.setDirtyCanvas?.(true, true);
              });
            };
            if (modelTypeWidget) {
              const oldModelTypeCallback = modelTypeWidget.callback;
              modelTypeWidget.callback = (v) => {
                oldModelTypeCallback?.(v);
                syncRefStatus();
              };
            }
            ...
            hideTrackRows();
            syncRefStatus();
            // Ref2VA subject references are stored inline in timeline.characters.

            // Let the DOM finish laying out, then expand the LiteGraph node to
            // contain the Voice Reference panel and both prompt editors.
            requestAnimationFrame(() => {
              requestAnimationFrame(() => {
                const size = self.computeSize?.();
                if (size) {
      ...
      nodeType.prototype.onConfigure = function (info) {
        setTimeout(() => {
          for (const [name, fallback] of [["custom_width", 0], ["custom_height", 0]]) {
            ...
            w.options.max = 8192;
            w.options.step = 1;
            if (w.value === undefined || w.value === null || Number.isNaN(Number(w.value))) w.value = fallback;
            if (Number(w.value) < 0) w.value = 0;
          }
        }, 0);
        ...
        const hardHideCompactNativeRows = () => {
          this._dhanH3HardHideTimingRows?.();
          if (Array.isArray(this.inputs)) {
            for (let i = this.inputs.length - 1; i >= 0; i--) {
              const input = this.inputs[i];
              if (["start_second", "end_second", "duration_seconds", "resize_method"].includes(input?.name) && input.link == null) {
                this.removeInput(i);
              }
            }
          }
        };
        // ComfyUI/Vue may rebuild widget-backed rows more than once during workflow restore.
        // Re-assert the compact layout after each restore pass so native duplicates never reappear.
        setTimeout(hardHideCompactNativeRows, 0);
        setTimeout(hardHideCompactNativeRows, 50);
        setTimeout(hardHideCompactNativeRows, 250);
...
// --- Comfyui-DHan-Minimax H3 Settings preset UI ---
app.registerExtension({
  name: "DHan.H3.SamplingPresetUI",
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData?.name !== "DHanH3SamplingPreset") return;

    const originalCreated = nodeType.prototype.onNodeCreated;
    nodeType.prototype.onNodeCreated = function() {
      const out = originalCreated?.apply(this, arguments);
      ...
      const syncPresetUI = () => {
        ...
        const customNames = [
          "custom_sampler", "custom_scheduler", "custom_steps",
          "shift_video", "shift_audio"
        ];
        ...
        for (const name of customNames) {
          ...
          if (!w) continue;
          if (!w.options) w.options = {};
          w.options.disabled = !custom;
        }

        const values = {
          "Standard":      {steps:20, scheduler:"beta"},
          "Turbo 4-Step": {steps:4,  scheduler:"simple"},
          "Turbo 6-Step": {steps:6,  scheduler:"simple"},
          "Turbo 8-Step": {steps:8,  scheduler:"simple"},
        }[preset.value];

        if (values && !custom) {
          const st = this.widgets?.find(w => w.name === "custom_steps");
          const sc = this.widgets?.find(w => w.name === "custom_scheduler");
          if (st) st.value = values.steps;
          if (sc) sc.value = values.scheduler;
        }
...
// --- Comfyui-DHan-Minimax H3 Preview Override UI ---
(() => {
  const EVENT = "dhan_h3_preview";
  ...
  function installListener() {
    ...
    api.addEventListener(EVENT, (event) => {
      ...
      // Prefer the exact Preview node ID. Some ComfyUI builds expose hidden
      // unique_id values differently for nodes invoked indirectly by Auto Render,
      // so when there is exactly one DHan preview node, use it as a safe fallback.
      let node = previewNodes.get(String(data.node_id));
      if (!node && previewNodes.size === 1) {
        node = [...previewNodes.values()][0];
      }
      if (!node || !node._dhanH3PreviewImg) {
        console.warn("[Comfyui-DHan-H3 Preview] preview packet had no matching node", data);
        return;
      }

      const img = node._dhanH3PreviewImg;
      const status = node._dhanH3PreviewStatus;

      if (data.webp) {
        // Replacing src restarts the animated WebP with the newest sampling state.
        img.src = `data:image/webp;base64,${data.webp}`;
      ...
      if (status) {
        ...
        status.textContent =
          `${step}/${total} · ${frames} preview frames · ${fps.toFixed(1)} fps · ${data.mode || ""}`;
      }
    ...
    api.addEventListener("dhan_h3_preview_status", (event) => {
      const data = event?.detail ?? event;
      if (!data) return;
      let node = previewNodes.get(String(data.node_id));
      if (!node && previewNodes.size === 1) {
      ...
      if (!node || !node._dhanH3PreviewStatus) return;
      node._dhanH3PreviewStatus.textContent = data.status || "Preview active";
      console.log("[Comfyui-DHan-H3 Preview]", data.status, data);
      app.graph?.setDirtyCanvas?.(true, true);
    });
  }

  app.registerExtension({
    name: "DHan.MiniMaxH3.PreviewOverrideUI",
    async beforeRegisterNodeDef(nodeType, nodeData) {
      if (nodeData?.name !== "DHanMiniMaxH3PreviewOverride") return;
      installListener();

      const oldCreated = nodeType.prototype.onNodeCreated;
      nodeType.prototype.onNodeCreated = function() {
        const out = oldCreated?.apply(this, arguments);
        const node = this;

        const wrap = document.createElement("div");
        wrap.style.cssText =
          "width:100%;box-sizing:border-box;padding:6px;background:#111;" +
          "border:1px solid #333;border-radius:5px;overflow:hidden;";

        const img = document.createElement("img");
        img.style.cssText =
          "display:none;width:100%;height:260px;object-fit:contain;background:#0a0a0a;" +
          "border-radius:3px;";
        img.alt = "MiniMax H3 live preview";

        const status = document.createElement("div");
        status.textContent = "Waiting for H3 sampling…";
        status.style.cssText =
          "padding-top:5px;font-size:10px;color:#999;text-align:center;white-space:nowrap;" +
          "overflow:hidden;text-overflow:ellipsis;";

        wrap.appendChild(img);
        wrap.appendChild(status);

        node._dhanH3PreviewImg = img;
        node._dhanH3PreviewStatus = status;
        previewNodes.set(String(node.id), node);

        const widget = node.addDOMWidget("h3_preview", "h3_preview", wrap, {
          getValue: () => "",
          setValue: () => {},
          serialize: false,
        });
        widget.computeSize = function(width) {
          return [Math.max(220, (node.size?.[0] || width || 360) - 20), 292];
        };

        const oldRemoved = node.onRemoved;
        node.onRemoved = function() {
          previewNodes.delete(String(node.id));
          oldRemoved?.apply(this, arguments);
        };

        setTimeout(() => {
          const size = node.computeSize?.();
          if (size) node.setSize?.([Math.max(node.size?.[0] || 360, 360), size[1]]);
          app.graph?.setDirtyCanvas?.(true, true);
        }, 0);
...
// --- Comfyui-DHan-H3 Storyboard Enhancer UI ---
(() => {
  const EVENT = "dhan_h3_prompt_enhancer_result";
  const nodes = new Map();
  let installed = false;
  const directors = () => (app.graph?._nodes || []).filter(n => n?.type === "DHanMiniMaxH3Director");
  const setW = (node,w,v) => { if(!w)return; const o=w.value; w.value=v; try{node.onWidgetChanged?.(w.name,v,o,w)}catch(_){} try{w.callback?.(v)}catch(_){} app.graph?.change?.(); app.graph?.setDirtyCanvas?.(true,true); };
  async function upload(file){ const b=new FormData(); b.append("image",file); b.append("type","input"); b.append("subfolder","whatdreamscost/storyboard"); b.append("overwrite","true"); const r=await api.fetchApi("/upload/image",{method:"POST",body:b}); if(!r.ok)throw new Error(`Upload failed: ${r.status}`); const x=await r.json(); return [x.subfolder,x.name].filter(Boolean).join("/"); }
  function install(){ if(installed)return; installed=true;

    // Keep Storyboard Enhancers visually active. During prompt serialization,
    // disable them for a normal workflow queue and enable only a specifically
    // requested enhancer for its Run Storyboard action.
    if(!app._dhanStoryboardQueueWrapped && typeof app.queuePrompt==="function"){
      app._dhanStoryboardQueueWrapped=true;
      const originalQueuePrompt=app.queuePrompt.bind(app);
      app.queuePrompt=async function(number,batchCount,options){
        const ids=(options?.queueNodeIds||[]).map(String);
        const armed=[];
        try{
          for(const n of nodes.values()){
            armed.push([n,n.mode]);
            n.mode=ids.includes(String(n.id))?0:2;
          }
          return await originalQueuePrompt(number,batchCount,options);
        }finally{
          for(const [n] of armed)n.mode=0;
          app.graph?.setDirtyCanvas?.(true,true);
        }
      };
    }

    api.addEventListener(EVENT,e=>{const d=e?.detail??e;let n=nodes.get(String(d?.node_id));if(!n&&nodes.size===1)n=[...nodes.values()][0];if(!n||!d)return;n._sbResult=d;if(!n.properties)n.properties={};try{n.properties.dhanStoryboardLastResult=JSON.stringify(d)}catch(_){}if(n._sbGlobal)n._sbGlobal.value=d.global_prompt||"";if(n._sbResultsTitle)n._sbResultsTitle.textContent=`RESULTS${d.timing_mode?` · ${d.timing_mode}`:""}`;const c=n._sbCards;if(c){c.innerHTML="";(d.items||[]).forEach(item=>{const card=document.createElement("div");card.style.cssText="padding:7px;border:1px solid #303030;border-radius:4px;background:#171717;font-size:10px;color:#ccc";const h=document.createElement("div");h.style.cssText="display:flex;justify-content:space-between;color:#8fa0b4;margin-bottom:4px";h.innerHTML=`<b>Part ${item.part}</b><span>${Number(item.start).toFixed(2)}–${Number(item.end).toFixed(2)}s · ${Number(item.duration).toFixed(2)}s</span>`;const p=document.createElement("div");p.textContent=item.prompt||"";p.style.whiteSpace="pre-wrap";card.append(h,p);const imgs=Array.isArray(item.images)?item.images:(item.imageFile?[item.imageFile]:[]);if(imgs.length){const q=document.createElement("div");q.textContent=`🖼 ${imgs.length} reference${imgs.length===1?"":"s"}`;q.style.cssText="margin-top:5px;color:#8aa98a;font-size:9px";card.appendChild(q)}c.appendChild(card)})}n._dhanUpdateApply?.();app.graph?.setDirtyCanvas?.(true,true);}); }
  app.registerExtension({name:"DHan.H3.StoryboardEnhancerUI",async beforeRegisterNodeDef(nodeType,nodeData){if(nodeData?.name!=="DHanH3PromptEnhancer")return;install();
  const protoConfigure=nodeType.prototype.onConfigure;
  nodeType.prototype.onConfigure=function(info){
    if(info?.properties){
      this._dhanPendingStoryboardProps={...info.properties};
    }
    const r=protoConfigure?.apply(this,arguments);
    this.mode=0;
    setTimeout(()=>{this.mode=0;this._dhanRestoreStoryboardState?.()},0);
    setTimeout(()=>{this.mode=0;this._dhanRestoreStoryboardState?.()},250);
    return r;
  };
  const old=nodeType.prototype.onNodeCreated;nodeType.prototype.onNodeCreated=function(){const out=old?.apply(this,arguments),node=this;if(!node.properties)node.properties={};if(node._dhanPendingStoryboardProps)node.properties={...node.properties,...node._dhanPendingStoryboardProps};

    // The queuePrompt wrapper suppresses this manual-only node during normal
    // workflow queues without leaving the editor permanently dimmed.
    node.mode=0;

    const hidden=name=>{const w=node.widgets?.find(x=>x.name===name);if(w){w.hidden=true;w.computeSize=()=>[0,-4];w.draw=()=>{}}return w};const ideaW=hidden("idea"),mapW=hidden("part_images_json");
    const wrap=document.createElement("div");wrap.style.cssText="width:100%;height:100%;min-height:0;box-sizing:border-box;padding:7px;background:#111;border:1px solid #333;border-radius:5px;display:flex;flex-direction:column;gap:7px;overflow:hidden";
    const bar=document.createElement("div");bar.style.cssText="display:flex;gap:6px;align-items:center;flex:none";const rm=document.createElement("button");rm.textContent="Refresh Models";rm.style.cssText="font-size:10px;padding:4px 9px;background:#222;color:#ddd;border:1px solid #444;border-radius:3px";rm.onclick=async()=>{const mw=node.widgets?.find(w=>w.name==="ollama_model"),uw=node.widgets?.find(w=>w.name==="ollama_url");try{const r=await api.fetchApi(`/dhan_h3/ollama_models?url=${encodeURIComponent(uw?.value||"http://127.0.0.1:11434")}`),d=await r.json(),ms=Array.isArray(d.models)?d.models:[];mw.options=mw.options||{};mw.options.values=ms.length?ms:["<refresh models>"];if(ms.length&&!ms.includes(mw.value))mw.value=ms[0]}catch(e){console.error(e)}};const ds=document.createElement("select");ds.style.cssText="margin-left:auto;min-width:170px;background:#222;color:#ddd;border:1px solid #444;border-radius:3px;padding:4px;font-size:10px";const rd=()=>{const prev=ds.value;ds.innerHTML="";directors().forEach(d=>{const o=document.createElement("option");o.value=String(d.id);o.textContent=`Director #${d.id}`;ds.appendChild(o)});if(prev&&[...ds.options].some(o=>o.value===prev))ds.value=prev};rd();const dr=document.createElement("button");dr.textContent="↻";dr.onclick=rd;
    const run=document.createElement("button");
    run.textContent="Run Storyboard";
    run.title="Execute only this Storyboard Enhancer";
    run.style.cssText="font-size:10px;padding:4px 8px;background:#26303a;color:#e1e6eb;border:1px solid #4a5663;border-radius:3px;cursor:pointer";
    run.onclick=async()=>{
      try{
        await app.queuePrompt(0,1,{queueNodeIds:[node.id],intent:"user"});
      }catch(e){
        console.error("[Comfyui-DHan-H3 Storyboard] manual queue failed",e);
      }
    };
    bar.append(rm,run,ds,dr);wrap.appendChild(bar);
    const il=document.createElement("div");il.textContent="IDEA";il.style.cssText="font-size:10px;font-weight:700;color:#8c9aaa";const idea=document.createElement("textarea");const savedIdea=node.properties?.dhanStoryboardIdea;idea.value=String((ideaW?.value&&String(ideaW.value).length?ideaW.value:savedIdea)||"");idea.placeholder="Describe the video/storyboard idea...";idea.style.cssText="width:100%;height:92px;min-height:54px;max-height:320px;resize:vertical;flex:none;box-sizing:border-box;background:#0f0f0f;color:#ddd;border:1px solid #333;border-radius:4px;padding:7px";idea.oninput=()=>{if(!node.properties)node.properties={};node.properties.dhanStoryboardIdea=idea.value;setW(node,ideaW,idea.value)};
    if(ideaW){
      ideaW.value=idea.value;
      ideaW.serializeValue=()=>idea.value;
    }
    wrap.append(il,idea);
    const ih=document.createElement("div");ih.style.cssText="display:flex;align-items:center;flex:none";const it=document.createElement("div");it.textContent="IMAGE REFERENCES";it.style.cssText="font-size:10px;font-weight:700;color:#8c9aaa";const add=document.createElement("button");add.textContent="+ Add Image";add.style.cssText="margin-left:auto;font-size:9px;padding:3px 7px;background:#222;color:#ddd;border:1px solid #444;border-radius:3px";const picker=document.createElement("input");picker.type="file";picker.accept="image/*";picker.multiple=true;picker.style.display="none";ih.append(it,add,picker);wrap.appendChild(ih);
    const drop=document.createElement("div");drop.style.cssText="flex:0 0 auto;max-height:120px;overflow:auto;padding:6px;border:1px dashed #3a3a3a;border-radius:4px;background:#151515;display:flex;flex-direction:column;gap:5px";wrap.appendChild(drop);node._sbRefs=[];
    const restoreSavedRefs=()=>{
      let raw=mapW?.value;
      if((raw===undefined||raw===null||raw===""||raw==="[]")&&node.properties?.dhanStoryboardImageRefs){
        raw=node.properties.dhanStoryboardImageRefs;
      }
      try{
        const a=typeof raw==="string"?JSON.parse(raw):(Array.isArray(raw)?raw:[]);
        if(Array.isArray(a)){
          node._sbRefs=a.filter(x=>x?.file).map(x=>({
            file:String(x.file),
            part:Math.max(1,parseInt(x.part||1))
          }));
        }
      }catch(_){}
    };
    restoreSavedRefs();
    let refsHydrating=true;const pc=()=>Math.max(1,Math.min(10,parseInt(node.widgets?.find(w=>w.name==="parts")?.value||1)));const sync=()=>{if(refsHydrating)return;const raw=JSON.stringify(node._sbRefs||[]);if(!node.properties)node.properties={};node.properties.dhanStoryboardImageRefs=raw;setW(node,mapW,raw)};const draw=()=>{drop.innerHTML="";const n=pc();node._sbRefs=node._sbRefs.map(r=>({...r,part:Math.max(1,Math.min(n,r.part||1))}));sync();if(!node._sbRefs.length){const e=document.createElement("div");e.textContent="Paste, drag images here, or click + Add Image";e.style.cssText="padding:8px;text-align:center;color:#666;font-size:9px";drop.appendChild(e);return}node._sbRefs.forEach((r,i)=>{const row=document.createElement("div");row.style.cssText="display:grid;grid-template-columns:48px minmax(0,1fr) 82px 24px;gap:6px;align-items:center;padding:4px;background:#1b1b1b;border:1px solid #303030;border-radius:4px";const im=document.createElement("img");im.style.cssText="width:48px;height:38px;object-fit:cover;border-radius:3px";const ps=r.file.replace(/\\/g,"/").split("/"),fn=ps.pop(),sf=ps.join("/");im.src=api.apiURL(`/view?filename=${encodeURIComponent(fn)}&type=input&subfolder=${encodeURIComponent(sf)}`);const name=document.createElement("div");name.textContent=r.file;name.title=r.file;name.style.cssText="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#aaa;font-size:9px";const sel=document.createElement("select");sel.style.cssText="background:#222;color:#ddd;border:1px solid #444;border-radius:3px;font-size:9px";for(let p=1;p<=n;p++){const o=document.createElement("option");o.value=p;o.textContent=`Part ${p}`;sel.appendChild(o)}sel.value=r.part;sel.onchange=()=>{r.part=parseInt(sel.value);sync()};const x=document.createElement("button");x.textContent="×";x.onclick=()=>{node._sbRefs.splice(i,1);draw()};row.append(im,name,sel,x);drop.appendChild(row)})};const files=async fs=>{for(const f of fs){if(!f?.type?.startsWith("image/"))continue;node._sbRefs.push({file:await upload(f),part:1})}draw()};add.onclick=()=>picker.click();picker.onchange=async()=>{await files([...picker.files]);picker.value=""};drop.ondragover=e=>e.preventDefault();drop.ondrop=async e=>{e.preventDefault();await files([...e.dataTransfer.files])};let hover=false;drop.onmouseenter=()=>hover=true;drop.onmouseleave=()=>hover=false;const paste=async e=>{if(!hover&&!wrap.contains(document.activeElement))return;const fs=[];for(const q of [...(e.clipboardData?.items||[])])if(q.type?.startsWith("image/")){const f=q.getAsFile();if(f)fs.push(f)}if(fs.length){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();await files(fs)}};window.addEventListener("paste",paste,true);restoreSavedRefs();draw();
    requestAnimationFrame(()=>{restoreSavedRefs();refsHydrating=false;draw()});
    setTimeout(()=>{restoreSavedRefs();refsHydrating=false;draw()},250);
    const persistAuthoringWidgets=()=>{if(!node.properties)node.properties={};for(const key of ["parts","duration_seconds","timing_mode","detail_level","ollama_model","ollama_url","seed","max_image_size"]){const w=node.widgets?.find(x=>x.name===key);if(w)node.properties[`dhanStoryboard_${key}`]=w.value}};
    const restoreAuthoringWidgets=()=>{if(!node.properties)return;for(const key of ["parts","duration_seconds","timing_mode","detail_level","ollama_model","ollama_url","seed","max_image_size"]){const w=node.widgets?.find(x=>x.name===key),v=node.properties[`dhanStoryboard_${key}`];if(w&&v!==undefined&&v!==null)w.value=v}};
    for(const key of ["duration_seconds","timing_mode","detail_level","ollama_model","ollama_url","seed","max_image_size"]){const w=node.widgets?.find(x=>x.name===key);if(w){const cb=w.callback;w.callback=function(){const r=cb?.apply(this,arguments);persistAuthoringWidgets();return r}}}
    const partsW=node.widgets?.find(w=>w.name==="parts");if(partsW){const cb=partsW.callback;partsW.callback=function(){const r=cb?.apply(this,arguments);persistAuthoringWidgets();setTimeout(draw,0);return r}};
    const results=document.createElement("div");results.style.cssText="flex:1 1 0;min-height:60px;overflow:hidden;border:1px solid #303030;border-radius:4px;display:flex;flex-direction:column";const title=document.createElement("div");title.textContent="RESULTS";node._sbResultsTitle=title;title.style.cssText="height:24px;flex:none;padding:6px;color:#8c9aaa;font-size:10px;font-weight:700;box-sizing:border-box";const scroll=document.createElement("div");scroll.style.cssText="flex:1 1 0;min-height:0;overflow:auto;padding:7px;display:flex;flex-direction:column;gap:7px";const gl=document.createElement("div");gl.textContent="Global Prompt";gl.style.cssText="font-size:9px;color:#8fa0b4;margin-bottom:2px";const g=document.createElement("textarea");g.readOnly=true;g.placeholder="Global Prompt";g.style.cssText="width:100%;height:68px;min-height:42px;max-height:180px;resize:vertical;box-sizing:border-box;background:#0a0a0a;color:#ddd;border:1px solid #333;border-radius:3px;padding:5px";node._sbGlobal=g;const tl=document.createElement("div");tl.textContent="Timed Parts";tl.style.cssText="font-size:9px;color:#8fa0b4;margin-top:2px";const cards=document.createElement("div");cards.style.cssText="display:flex;flex-direction:column;gap:5px;min-height:0";node._sbCards=cards;scroll.append(gl,g,tl,cards);results.append(title,scroll);wrap.appendChild(results);
    const footer=document.createElement("div");footer.style.cssText="flex:0 0 auto;display:flex;position:relative;z-index:2";const apply=document.createElement("button");apply.textContent="Apply Storyboard to Director";apply.style.cssText="width:100%;padding:7px;border-radius:4px;font-weight:700";const updateApply=()=>{const ready=Boolean(node._sbResult?.items?.length);apply.disabled=!ready;apply.style.background=ready?"#1677c8":"#24282c";apply.style.color=ready?"#fff":"#737b84";apply.style.border=ready?"1px solid #48a8f0":"1px solid #3b4147";apply.style.cursor=ready?"pointer":"not-allowed";apply.title=ready?"Apply this storyboard to the selected Director":"Run Storyboard first"};node._dhanUpdateApply=updateApply;updateApply();apply.onclick=()=>{const d=node._sbResult;if(!d?.items?.length)return;rd();const director=directors().find(x=>Number(x.id)===parseInt(ds.value)),ed=director?._timelineEditor;if(!ed){alert("Target Director is not ready.");return}if(ed._dhanH3Mode==="Ref2VA"){alert("Storyboard Apply currently supports FL2VA only.");return}const fps=ed.getFrameRate?.()||24,total=Math.max(1,Math.round(Number(d.duration_seconds||1)*fps));const w=(x,v)=>{if(!x)return;const o=x.value;x.value=v;try{director.onWidgetChanged?.(x.name,v,o,x)}catch(_){}try{x.callback?.(v)}catch(_){}};w(ed.startFramesWidget,0);w(ed.startSecondsWidget,0);w(ed.durationFramesWidget,total);w(ed.durationSecondsWidget,Number(d.duration_seconds||0));w(ed.endFramesWidget,total);w(ed.endSecondsWidget,Number(d.duration_seconds||0));ed.timeline.global_prompt=d.global_prompt||"";if(ed.globalPromptInput)ed.globalPromptInput.value=d.global_prompt||"";try{ed.syncGlobalPrompt?.(d.global_prompt||"")}catch(_){}ed.timeline.segments=d.items.map((item,i)=>{const a=Math.round(Number(item.start)*fps),b=Math.max(a+1,Math.round(Number(item.end)*fps)),imgs=Array.isArray(item.images)?item.images:[],file=String(imgs[0]||item.imageFile||"");const seg={id:`SB_${Date.now()}_${i}`,start:a,length:b-a,prompt:String(item.prompt||""),type:file?"image":"text"};if(file){seg.imageFile=file;const ps=file.replace(/\\/g,"/").split("/"),fn=ps.pop(),sf=ps.join("/");const im=new Image();im.onload=()=>ed.render?.();im.src=api.apiURL(`/view?filename=${encodeURIComponent(fn)}&type=input&subfolder=${encodeURIComponent(sf)}`);seg.imgObj=im}return seg});ed.selectedSegmentIds=[];ed._selectedIndex=-1;ed.commitChanges?.(true);ed.render?.();app.graph?.change?.();apply.textContent="Applied ✓";setTimeout(()=>apply.textContent="Apply Storyboard to Director",1200)};footer.appendChild(apply);wrap.appendChild(footer);
    const renderSavedResult=d=>{if(!d||typeof d!=="object")return;node._sbResult=d;if(node._sbGlobal)node._sbGlobal.value=d.global_prompt||"";if(node._sbResultsTitle)node._sbResultsTitle.textContent=`RESULTS${d.timing_mode?` · ${d.timing_mode}`:""}`;if(node._sbCards){node._sbCards.innerHTML="";(d.items||[]).forEach(item=>{const card=document.createElement("div");card.style.cssText="padding:7px;border:1px solid #303030;border-radius:4px;background:#171717;font-size:10px;color:#ccc";const h=document.createElement("div");h.style.cssText="display:flex;justify-content:space-between;color:#8fa0b4;margin-bottom:4px";h.innerHTML=`<b>Part ${item.part}</b><span>${Number(item.start).toFixed(2)}–${Number(item.end).toFixed(2)}s · ${Number(item.duration).toFixed(2)}s</span>`;const p=document.createElement("div");p.textContent=item.prompt||"";p.style.whiteSpace="pre-wrap";card.append(h,p);const imgs=Array.isArray(item.images)?item.images:(item.imageFile?[item.imageFile]:[]);if(imgs.length){const q=document.createElement("div");q.textContent=`🖼 ${imgs.length} reference${imgs.length===1?"":"s"}`;q.style.cssText="margin-top:5px;color:#8aa98a;font-size:9px";card.appendChild(q)}node._sbCards.appendChild(card)})}updateApply()};
    const restoreStoryboardState=()=>{try{refsHydrating=true;if(node._dhanPendingStoryboardProps)node.properties={...node.properties,...node._dhanPendingStoryboardProps};restoreAuthoringWidgets();const saved=node.properties?.dhanStoryboardIdea;if(saved!==undefined&&saved!==null){idea.value=String(saved);if(ideaW)setW(node,ideaW,String(saved))}restoreSavedRefs();refsHydrating=false;draw();const raw=node.properties?.dhanStoryboardLastResult;if(raw){renderSavedResult(typeof raw==="string"?JSON.parse(raw):raw)}}catch(e){console.warn("[Comfyui-DHan-H3 Storyboard] state restore failed",e)}};
    node._dhanRestoreStoryboardState=restoreStoryboardState;
    restoreStoryboardState();
    nodes.set(String(node.id),node);const widget=node.addDOMWidget(
      "h3_storyboard_result","h3_storyboard_result",wrap,
      {getValue:()=>"",setValue:()=>{},serialize:false}
    );

    // Same resize architecture as the working ComfyUI Inline Image node:
    // - report only a stable minimum to ComfyUI;
    // - never use node.size inside computeSize();
    // - size the internal viewport directly from node.size inside onResize();
    // - clamp only genuinely unusable minimum node dimensions.
    const SB_MIN_NODE_W = 470;
    const SB_MIN_NODE_H = 430;
    const SB_MIN_VIEW_H = 190;
    const SB_BOTTOM_PAD = 48;
    let sbResizing = false;

    widget.computeSize = () => [SB_MIN_NODE_W - 20, 250];

    function setStoryboardSizeFromNode() {
      const top = Math.max(110, Number(widget.last_y || 150));
      const available = Math.max(
        SB_MIN_VIEW_H,
        Math.floor(Number(node.size?.[1] || SB_MIN_NODE_H) - top - SB_BOTTOM_PAD)
      );

      // Only size our own content viewport. Do not touch parent DOM wrappers and do
      // not call node.setSize() here. This mirrors InlineImage's setCanvasSizeFromNode().
      wrap.style.height = `${available}px`;
      wrap.style.minHeight = '0px';
      wrap.style.maxHeight = 'none';
      wrap.style.overflow = 'hidden';

      // Results is the elastic region. Fixed authoring rows + footer remain unchanged.
      results.style.flex = '1 1 0';
      results.style.minHeight = '60px';
      scroll.style.flex = '1 1 0';
      scroll.style.minHeight = '0px';
      scroll.style.overflow = 'auto';
    }

    function enforceStoryboardNodeSize(size) {
      if (sbResizing) return;
      sbResizing = true;
      if (size[0] < SB_MIN_NODE_W) size[0] = node.size[0] = SB_MIN_NODE_W;
      if (size[1] < SB_MIN_NODE_H) size[1] = node.size[1] = SB_MIN_NODE_H;
      sbResizing = false;
    }
    ...
    node.onResize=function(size){
      ...
      if (sbResizing) return;
      enforceStoryboardNodeSize(size);
      setStoryboardSizeFromNode();
      app.graph?.setDirtyCanvas?.(true,true);
    };

    const oldSerialize=node.onSerialize;node.onSerialize=function(o){try{if(!node.properties)node.properties={};node.properties.dhanStoryboardIdea=idea.value;if(ideaW)setW(node,ideaW,idea.value);persistAuthoringWidgets();sync();if(node._sbResult)node.properties.dhanStoryboardLastResult=JSON.stringify(node._sbResult)}catch(_){}return oldSerialize?.apply(this,arguments)};
    const rem=node.onRemoved;
    node.onRemoved=function(){
      nodes.delete(String(node.id));
      window.removeEventListener("paste",paste,true);
      rem?.apply(this,arguments)
    };

    // Same startup pattern as InlineImage: establish only the true minimum, then
    // immediately size the internal viewport from the resulting node dimensions.
    requestAnimationFrame(()=>{
      node.setSize?.([
        Math.max(node.size?.[0] || SB_MIN_NODE_W, SB_MIN_NODE_W),
        Math.max(node.size?.[1] || SB_MIN_NODE_H, SB_MIN_NODE_H)
      ]);
      setStoryboardSizeFromNode();
      app.graph?.setDirtyCanvas?.(true,true);
    });
