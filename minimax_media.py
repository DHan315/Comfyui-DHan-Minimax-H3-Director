...
def video_dimensions(file_ref: str):
    """Return the encoded video dimensions without decoding its frames."""
    ...
    try:
        with av.open(path) as container:
            ...
            width = stream.width or stream.codec_context.width
            height = stream.height or stream.codec_context.height
            if width and height:
                return int(width), int(height)
    except Exception as e:
        log.warning("[MiniMaxDirector] Could not read video dimensions for %s: %s", file_ref, e)
    return None
